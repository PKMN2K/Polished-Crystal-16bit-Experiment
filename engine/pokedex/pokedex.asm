...
_Pokedex_MonHasCosmeticForms:
	...
	ld de, -VariantVisualSpeciesAndFormTable
...
.mon_caught
	...
	call GetBodyDataPointerFromIndexBC
...
; It uses bc as input for species+form, de as the save-compatible flag index.
...
; being checked, and de contains its save-compatible Pokédex flag index.
...
.species_loop
	...
	; Iterate through the visual form table to find entries matching this species.
	; NUM_SPECIES is the first form-table index; GetPokedexFlagIndex translates
	; each matching entry to its save-compatible physical flag index.
	ld de, NUM_SPECIES
	push hl
.form_loop
...
.got_types
	...
	call Pokedex_GetCursorSpecies
	call GetSpeciesAndFormIndex
	inc bc ; zero-based legacy data index -> one-based native species ID
