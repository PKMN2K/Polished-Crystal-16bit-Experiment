...
; sets carry if exited menu.
	...
	rst AddNTimes
	farcall LoadCurSpeciesAndFormFromPokemonDataStruct
	; Selection historically publishes the full form byte, including Egg and
	; gender metadata; restore those non-identity bits after species decoding.
	push hl
	ld de, MON_FORM - MON_SPECIES
	add hl, de
	ld a, [hl]
	and (GENDER_MASK | IS_EGG_MASK)
	ld b, a
	ld a, [wCurForm]
	or b
	ld [wCurForm], a
...
.Text_RecoveredSomeHP:
	; recovered @ HP!
	text_farend _RecoveredSomeHPText
.Text_CuredOfPoison:
	; 's cured of poison.
	text_farend _CuredOfPoisonText
.Text_RidOfParalysis:
	; 's rid of paralysis.
	text_farend _RidOfParalysisText
.Text_BurnWasHealed:
	; 's burn was healed.
	text_farend _BurnWasHealedText
.Text_Defrosted:
	; was defrosted.
	text_farend _WasDefrostedText
.Text_WokeUp:
	; woke up.
	text_farend _WokeUpText
.Text_HealthReturned:
	; 's health returned.
	text_farend _HealthReturnedText
.Text_Revitalized:
	; is revitalized.
	text_farend _RevitalizedText
.Text_GrewToLevel:
	; grew to level @ !@ @
	text_farend _GrewToLevelText
.Text_CameToItsSenses:
	; came to its senses.
	text_farend _CameToItsSensesText
