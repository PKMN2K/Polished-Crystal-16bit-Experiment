...
.got_wild_table_type
	...
	jr nz, .grass_vars
	ld a, NUM_WATERMON
	lb de, WATER_WILDDATA_LENGTH, 4 ; first surf species follows map/rate/level
	jr .got_vars

.grass_vars
	ld a, 4 - NUM_GRASSMON * WILDMON_DATA_LENGTH
	inc e
.loop
	add NUM_GRASSMON * WILDMON_DATA_LENGTH
	dec e
	jr nz, .loop
	ld e, a
...
.GrassTables:
	dba JohtoGrassWildMons
	dba KantoGrassWildMons
	dba OrangeGrassWildMons

.SurfTables:
	dba JohtoWaterWildMons
	dba KantoWaterWildMons
	dba OrangeWaterWildMons
...
_ChooseWildEncounter:
	...
	push bc
	ld bc, NUM_GRASSMON * WILDMON_DATA_LENGTH
	rst AddNTimes
	pop bc
...
GetWildDataByte:
; Read from the active wild-data table without assuming its ROM bank.
	ld a, [wWildMonDataBank]
	jp GetFarByte

LoadLegacyWildmonFromHL:
; Read a native species word and form from the active wild-data bank, then
; resolve them to Polished's transitional root-species/form representation.
; in: hl = native species word
; out: a = c = root species byte, b = encoded form, hl = next record
	call GetWildDataByte
	inc hl
	ld c, a
	call GetWildDataByte
...
LoadLegacySpeciesAndFormFromHL:
; Read a native species word and raw form from the current ROM bank, then
; resolve them to Polished's transitional root-species/form representation.
; in: hl = native species word
; out: a = c = root species byte, b = encoded form, hl = next byte
	ld a, [hli]
	ld c, a
	ld a, [hli]
	ld b, a
...
; species
	...
	farcall StoreRoamMonNativeSpecies
	ld bc, ENTEI
...
; We've decided to take on a beast, so stage its information for battle.
	...
	dec hl
	dec hl
	farcall GetLegacyRoamMonSpeciesAndForm
	ld a, d
	ld [wTempWildMonSpecies], a
	ld a, e
	ld [wWildMonForm], a
...
.GetGrassmon:
	push hl
	ld bc, 2 + 1 + 4 * WILDMON_DATA_LENGTH
	add hl, bc
	call GetTimeOfDayNotEve
	ld bc, NUM_GRASSMON * WILDMON_DATA_LENGTH
	rst AddNTimes
...
; We now have the pointer to one of the last (rarest) three wild Pokemon found in that area.
	...
	ld [wCurForm], a
	pop hl
	ld de, 2 + 1 + 0 * WILDMON_DATA_LENGTH
	add hl, de
...
.ok
	ld bc, 2 + 1 + 0 * WILDMON_DATA_LENGTH
	add hl, bc
	call GetTimeOfDayNotEve
	ld bc, NUM_GRASSMON * WILDMON_DATA_LENGTH
	rst AddNTimes
...
; Get a random monster owned by the trainer who's calling.
	...
	; All trainers use at least 4 bytes per mon
	; (Level, 16-bit native species, Form).
	ld c, 4
...
SECTION "Johto Grass Wild Data", ROMX

JohtoGrassWildMons:
INCLUDE "data/wild/johto_grass.asm"

SECTION "Johto Water Wild Data", ROMX

JohtoWaterWildMons:
INCLUDE "data/wild/johto_water.asm"

SECTION "Kanto Grass Wild Data", ROMX

KantoGrassWildMons:
INCLUDE "data/wild/kanto_grass.asm"

SECTION "Kanto Water Wild Data", ROMX

KantoWaterWildMons:
INCLUDE "data/wild/kanto_water.asm"

SECTION "Orange Grass Wild Data", ROMX

OrangeGrassWildMons:
INCLUDE "data/wild/orange_grass.asm"

SECTION "Orange Water Wild Data", ROMX

OrangeWaterWildMons:
INCLUDE "data/wild/orange_water.asm"

SECTION "Swarm Grass Wild Data", ROMX

SwarmGrassWildMons:
INCLUDE "data/wild/swarm_grass.asm"

SECTION "Swarm Water Wild Data", ROMX
