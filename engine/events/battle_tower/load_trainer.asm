DEF BTMON_SIZE EQU 10 ; native species + form/gender + item + 4 moves + DV index + personality ability/nature
...
; Loads the 6 player rental Pokémon in wBT_MonParty for party lineup.
	...
	rst CopyBytes
	jr ConvertBattleTowerPlayerPartyForStorage
...
; Prepares your and the opponent's parties for battle tower battle.
	...
	ld bc, wPartyMonNicknamesEnd - wPartyCount
	rst CopyBytes
	call ConvertBattleTowerPlayerPartyForStorage
...
ConvertBattleTowerPlayerPartyForStorage:
; Tower/rental records are constructed in legacy OT-party format. Convert the
; completed player-party copies only when transient persistent storage is active.
	ld a, [wPartyCount]
	and a
	ret z
	ld b, a
...
.loop
	push bc
	farcall PrepareLegacyPokemonDataStructForStorage
...
.got_mon_index
	...
	farcall LoadLegacySpeciesAndFormFromHL
	...
	; Add the decoded species and form to the transitional party structure.
	push hl
	ld hl, MON_SPECIES
	add hl, de
