"""Eighteenth real move-script command / posthiteffects native-identity regression.

Execute the validated first-turn wild Tackle path through real
postfainteffects, then real BattleCommand_posthiteffects. Both battlers remain
alive, the hit succeeds, and the fixture has no substitute, reactive item, or
Parental Bond follow-up. The following endmove byte is replaced with
endturn_command so this checkpoint stops immediately after posthiteffects.
"""
import argparse
...
def main():
    ...
    try:
        ...
        # postfainteffects and posthiteffects real. Replace only the following
        # endmove command with endturn_command ($fe), so this checkpoint
        # completes the eighteenth real command and stops before endmove.
        ...
        mem[normal_bank, normal_addr + 18] = 0xFE
        ...
        for native, species, form in cases:
            ...
            assert len(read_script_calls) == 19, (
                "posthiteffects did not return to the terminal script byte",
                context, len(read_script_calls)
            )
            assert read_script_snapshots[18][:5] == (25, native, 0, 33, 0), (
                "native/move/hit state changed after posthiteffects",
                context, read_script_snapshots
            )
            assert read_script_snapshots[18][5:] == advance_script_pointer(
                read_script_snapshots[0], 18
            ), (
                "script pointer did not advance through eighteen real commands",
                context, read_script_snapshots
            )
            assert bytes(
        ...
        print(
            f"PASS: {count} native posthiteffects cases; real "
            ...
            "real posthiteffects return -> nineteenth script-byte terminal "
            "before endmove"
        )
    finally:
