"""Native HandleFutureSight zero-counter -> following CheckFaint regression.

Execute the validated player-first wild Tackle path through both real move
turns, the early between-turn checks, weather, and HandleAffectionSelfCure.

Then execute the real HandleFutureSight body with both Future Sight counters
zero. Its native SetFastestTurn/.do_it/SwitchTurn/.do_it flow must visit both
battlers, and each .do_it must take the zero-counter RET Z before any delayed
move processing. Stop exactly at the following CheckFaint entry before that
faint check executes, while preserving native identity, PP, Pressure, party HP,
damage bookkeeping, wMoveState, wBattleEnded, wEnemyFleeing, and weather state.
"""
import argparse
...
def main():
    ...
    # BattleTurn.do_move is:
    #   call PerformMove                 ; +0
    #   ld a, [wBattleEnded]             ; +3
    #   and a / ret nz                   ; +6/+7
    #   call DeferredSwitch              ; +8
    #   call ResetAbilityIgnorance       ; +11
    #   ld a, [wBattleEnded]             ; +14
    #   and a / ret                      ; +17/+18
    # The controlled stop is the caller's following ProcessEnemyFleeing entry.
    do_move_bank, do_move_addr = symbols["BattleTurn.do_move"]
    do_move_offset = offset("BattleTurn.do_move")
    assert data[do_move_offset + 11] == 0xCD
    assert int.from_bytes(
        data[do_move_offset + 12:do_move_offset + 14], "little"
    ) == addr("ResetAbilityIgnorance")
    assert data[do_move_offset + 14] == 0xFA
    assert data[do_move_offset + 17] == 0xA7
    assert data[do_move_offset + 18] == 0xC9
    ...
    do_move_relative = do_move_addr - battle_turn_addr
    process_call_offsets = []
    for i in range(do_move_relative - 2):
        if (
            data[battle_turn_offset + i] == 0xCD
            and int.from_bytes(
                data[battle_turn_offset + i + 1:battle_turn_offset + i + 3],
                "little",
            ) == process_enemy_flee_addr
        ):
            process_call_offsets.append(i)
    assert len(process_call_offsets) == 1, process_call_offsets
    process_call_offset = process_call_offsets[0]
    post_process_flee_addr = battle_turn_addr + process_call_offset + 3
    assert data[battle_turn_offset + process_call_offset + 3] == 0xFA
    assert int.from_bytes(
        data[
            battle_turn_offset + process_call_offset + 4:
            battle_turn_offset + process_call_offset + 6
        ],
        "little",
    ) == addr("wBattleEnded")
    ...
    # HandleBetweenTurnEffects begins:
    #   call CheckFaint
    #   ret c
    #   call HandleWeather
    #   call CheckFaint
    ...
    between_turn_offset = offset("HandleBetweenTurnEffects")
    assert data[between_turn_offset] == 0xCD
    assert int.from_bytes(
        data[between_turn_offset + 1:between_turn_offset + 3], "little"
    ) == addr("CheckFaint")
    assert data[between_turn_offset + 3] == 0xD8
    assert data[between_turn_offset + 4] == 0xCD
    assert int.from_bytes(
        data[between_turn_offset + 5:between_turn_offset + 7], "little"
    ) == addr("HandleWeather")
    assert data[between_turn_offset + 7] == 0xCD
    assert int.from_bytes(
        data[between_turn_offset + 8:between_turn_offset + 10], "little"
    ) == addr("CheckFaint")
    assert data[between_turn_offset + 10] == 0xD8
    assert data[between_turn_offset + 11] == 0xCD
    assert int.from_bytes(
        data[between_turn_offset + 12:between_turn_offset + 14], "little"
    ) == addr("HandleAffectionSelfCure")
    assert data[between_turn_offset + 14] == 0xCD
    assert int.from_bytes(
        data[between_turn_offset + 15:between_turn_offset + 17], "little"
    ) == addr("HandleFutureSight")
    assert data[between_turn_offset + 17] == 0xCD
    assert int.from_bytes(
        data[between_turn_offset + 18:between_turn_offset + 20], "little"
    ) == addr("CheckFaint")

    # HandleFutureSight itself must retain the native two-side structure:
    #   call SetFastestTurn
    #   call .do_it
    #   call SwitchTurn
    future_sight_offset = offset("HandleFutureSight")
    assert data[future_sight_offset] == 0xCD
    assert int.from_bytes(
        data[future_sight_offset + 1:future_sight_offset + 3], "little"
    ) == addr("SetFastestTurn")
    assert data[future_sight_offset + 3] == 0xCD
    assert int.from_bytes(
        data[future_sight_offset + 4:future_sight_offset + 6], "little"
    ) == addr("HandleFutureSight.do_it")
    assert data[future_sight_offset + 6] == 0xCD
    assert int.from_bytes(
        data[future_sight_offset + 7:future_sight_offset + 9], "little"
    ) == addr("SwitchTurn")

    # HandleAffectionSelfCure itself must retain the native two-side structure:
    #   call SetFastestTurn
    #   call .do_it
    #   call SwitchTurn
    affection_offset = offset("HandleAffectionSelfCure")
    assert data[affection_offset] == 0xCD
    assert int.from_bytes(
        data[affection_offset + 1:affection_offset + 3], "little"
    ) == addr("SetFastestTurn")
    assert data[affection_offset + 3] == 0xCD
    assert int.from_bytes(
        data[affection_offset + 4:affection_offset + 6], "little"
    ) == addr("HandleAffectionSelfCure.do_it")
    assert data[affection_offset + 6] == 0xCD
    assert int.from_bytes(
        data[affection_offset + 7:affection_offset + 9], "little"
    ) == addr("SwitchTurn")


    # HandleWeather no-weather path:
    #   ld a, [wBattleWeather]
    #   and a
    #   ret z
    weather_offset = offset("HandleWeather")
    assert data[weather_offset] == 0xFA
    assert int.from_bytes(
        data[weather_offset + 1:weather_offset + 3], "little"
    ) == addr("wBattleWeather")
    assert data[weather_offset + 3] == 0xA7
    assert data[weather_offset + 4] == 0xC8

    # ProcessEnemyFleeing starts with:
    #   ld a, [wEnemyFleeing]
    #   and a
    #   ret z
    process_flee_offset = offset("ProcessEnemyFleeing")
    assert data[process_flee_offset] == 0xFA
    assert int.from_bytes(
        data[process_flee_offset + 1:process_flee_offset + 3], "little"
    ) == addr("wEnemyFleeing")
    assert data[process_flee_offset + 3] == 0xA7
    assert data[process_flee_offset + 4] == 0xC8
    ...
    enemy_critical_snapshots = []
    enemy_reset_crit_snapshots = []
    enemy_critical_opp_ability_snapshots = []
    enemy_future_sight_snapshots = []
    enemy_user_item_after_unnerve_snapshots = []
    enemy_user_item_snapshots = []
    enemy_critical_user_ability_snapshots = []
    enemy_critical_affection_calls = []
    enemy_critical_random_calls = []
    enemy_user_valid_item_calls = []
    enemy_critical_result_snapshots = []
    damagestats_active = [False]
    enemy_damagestats_active = [False]
    damagestats_calls = []
    ...
    enemy_damagestats_snapshots = []
    enemy_damage_reset_snapshots = []
    enemy_damage_opponent_attr_snapshots = []
    enemy_damage_user_attr_snapshots = []
    enemy_damagestats_future_sight_snapshots = []
    enemy_damage_screen_snapshots = []
    enemy_true_user_party_attr_snapshots = []
    enemy_damagestats_user_ability_snapshots = []
    enemy_damagestats_result_snapshots = []
    enemy_damagecalc_active = [False]
    enemy_damagecalc_snapshots = []
    enemy_damagecalc_result_snapshots = []
    enemy_stab_active = [False]
    enemy_stab_snapshots = []
    enemy_stab_type_matchup_snapshots = []
    enemy_stab_weather_snapshots = []
    enemy_stab_result_snapshots = []
    enemy_damagevariation_active = [False]
    enemy_damagevariation_snapshots = []
    enemy_damagevariation_random_calls = []
    enemy_damagevariation_result_snapshots = []
    enemy_moveanim_active = [False]
    enemy_moveanim_snapshots = []
    enemy_moveanim_lowersub_calls = []
    enemy_moveanim_nosub_calls = []
    enemy_moveanim_raisesub_calls = []
    enemy_moveanim_fx_calls = []
    enemy_moveanim_fx_snapshots = []
    enemy_moveanim_result_snapshots = []
    enemy_failuretext_snapshots = []
    enemy_failuretext_result_snapshots = []
    ...
    enemy_applydamage_active = [False]
    enemy_applydamage_snapshots = []
    enemy_applydamage_affection_calls = []
    enemy_applydamage_item_calls = []
    enemy_applydamage_ability_calls = []
    enemy_applydamage_reset_subhit_calls = []
    enemy_applydamage_check_sub_calls = []
    enemy_applydamage_take_damage_calls = []
    enemy_applydamage_deal_damage_calls = []
    enemy_applydamage_subtract_hp_calls = []
    enemy_applydamage_hud_calls = []
    enemy_applydamage_refresh_huds_calls = []
    enemy_applydamage_result_snapshots = []
    ...
    enemy_criticaltext_active = [False]
    enemy_criticaltext_snapshots = []
    enemy_criticaltext_checkcrit_calls = []
    enemy_criticaltext_checkcrit_snapshots = []
    enemy_criticaltext_delay_calls = []
    enemy_criticaltext_delay_snapshots = []
    enemy_criticaltext_result_snapshots = []
    ...
    enemy_supereffectivetext_active = [False]
    enemy_supereffectivetext_snapshots = []
    enemy_supereffectivetext_text_calls = []
    enemy_supereffectivetext_item_calls = []
    enemy_supereffectivetext_result_snapshots = []
    ...
    enemy_postfainteffects_active = [False]
    enemy_postfainteffects_snapshots = []
    enemy_postfaint_fainted_calls = []
    enemy_postfaint_run_faint_abilities_calls = []
    enemy_postfainteffects_result_snapshots = []
    posthiteffects_active = [False]
    posthiteffects_calls = []
    posthiteffects_snapshots = []
    posthiteffects_result_snapshots = []
    enemy_posthiteffects_active = [False]
    enemy_posthiteffects_snapshots = []
    enemy_posthiteffects_result_snapshots = []
    enemy_endmove_effect_calls = []
    ...
    post_future_checkfaint_stop_hit = [False]
    ...
    post_reset_calls = []
    process_enemy_fleeing_calls = []
    process_enemy_flee_return_calls = []
    handle_between_turn_effects_calls = []
    between_turn_checkfaint_calls = []
    between_turn_resolve_faints_calls = []
    handle_weather_calls = []
    post_weather_checkfaint_calls = []
    handle_affection_self_cure_calls = []
    affection_check_calls = []
    affection_random_calls = []
    handle_future_sight_calls = []
    future_sight_do_it_calls = []
    post_future_checkfaint_calls = []
    weather_deep_calls = []
    between_turn_active = [False]
    weather_active = [False]
    affection_self_cure_active = [False]
    ...
    def invoke(label, max_frames=128, stop_flag=None):
        ...
        # Controlled boundary sink used only after real HandleFutureSight
        # finishes both zero-counter sides and reaches the following CheckFaint.
        ...
        if stop_flag is not None and stop_flag[0]:
            # The stop is at the CheckFaint immediately following
            # HandleFutureSight, before that CheckFaint body executes.
            assert mem[addr("hROMBank")] == symbols["CheckFaint"][0], label
        else:
    ...
    def observe_read_script(_):
        ...
        if len(read_script_calls) == 27:
            enemy_critical_result_snapshots.append((
                read_native("wBattleMonNativeSpecies"),
                read_native("wEnemyMonNativeSpecies"),
                mem[addr("hBattleTurn")],
                mem[addr("wCurPlayerMove")],
            ...
            enemy_critical_active[0] = False
        if len(read_script_calls) == 28:
            enemy_damagestats_result_snapshots.append((
                read_native("wBattleMonNativeSpecies"),
                read_native("wEnemyMonNativeSpecies"),
                mem[addr("hBattleTurn")],
                mem[addr("wCurPlayerMove")],
            ...
            enemy_damagestats_active[0] = False
        if len(read_script_calls) == 29:
            enemy_damagecalc_result_snapshots.append((
                read_native("wBattleMonNativeSpecies"),
                read_native("wEnemyMonNativeSpecies"),
                mem[addr("hBattleTurn")],
                mem[addr("wCurPlayerMove")],
            ...
            enemy_damagecalc_active[0] = False
        if len(read_script_calls) == 30:
            enemy_stab_result_snapshots.append((
                read_native("wBattleMonNativeSpecies"),
                read_native("wEnemyMonNativeSpecies"),
                mem[addr("hBattleTurn")],
                mem[addr("wCurPlayerMove")],
            ...
            enemy_stab_active[0] = False
        if len(read_script_calls) == 31:
            enemy_damagevariation_result_snapshots.append((
                read_native("wBattleMonNativeSpecies"),
                read_native("wEnemyMonNativeSpecies"),
                mem[addr("hBattleTurn")],
                mem[addr("wCurPlayerMove")],
            ...
            enemy_damagevariation_active[0] = False
        if len(read_script_calls) == 32:
            enemy_moveanim_result_snapshots.append((
                read_native("wBattleMonNativeSpecies"),
                read_native("wEnemyMonNativeSpecies"),
                mem[addr("hBattleTurn")],
                mem[addr("wCurPlayerMove")],
            ...
            enemy_moveanim_active[0] = False
        if len(read_script_calls) == 33:
            enemy_failuretext_result_snapshots.append((
                read_native("wBattleMonNativeSpecies"),
                read_native("wEnemyMonNativeSpecies"),
                mem[addr("hBattleTurn")],
                mem[addr("wCurPlayerMove")],
            ...
            failuretext_active[0] = False
        if len(read_script_calls) == 34:
            enemy_applydamage_result_snapshots.append((
                read_native("wBattleMonNativeSpecies"),
                read_native("wEnemyMonNativeSpecies"),
                mem[addr("hBattleTurn")],
                mem[addr("wCurPlayerMove")],
            ...
            enemy_applydamage_active[0] = False
        if len(read_script_calls) == 35:
            enemy_criticaltext_result_snapshots.append((
                read_native("wBattleMonNativeSpecies"),
                read_native("wEnemyMonNativeSpecies"),
                mem[addr("hBattleTurn")],
                mem[addr("wCurPlayerMove")],
            ...
            enemy_criticaltext_active[0] = False
        if len(read_script_calls) == 36:
            enemy_supereffectivetext_result_snapshots.append((
                read_native("wBattleMonNativeSpecies"),
                read_native("wEnemyMonNativeSpecies"),
                mem[addr("hBattleTurn")],
                mem[addr("wCurPlayerMove")],
            ...
            enemy_supereffectivetext_active[0] = False
        if len(read_script_calls) == 37:
            enemy_postfainteffects_result_snapshots.append((
                read_native("wBattleMonNativeSpecies"),
                read_native("wEnemyMonNativeSpecies"),
                mem[addr("hBattleTurn")],
                mem[addr("wCurPlayerMove")],
            ...
            enemy_postfainteffects_active[0] = False
        if len(read_script_calls) == 38:
            enemy_posthiteffects_result_snapshots.append((
                read_native("wBattleMonNativeSpecies"),
                read_native("wEnemyMonNativeSpecies"),
                mem[addr("hBattleTurn")],
                mem[addr("wCurPlayerMove")],
    ...
    def observe_target_fainted(_):
        ...
        if enemy_postfainteffects_active[0]:
            enemy_postfaint_fainted_calls.append((
                read_native("wBattleMonNativeSpecies"),
                read_native("wEnemyMonNativeSpecies"),
                mem[addr("hBattleTurn")],
                mem[addr("wBattleMonHP")],
    ...
    def observe_target_ability(_):
        ...
        if enemy_critical_active[0]:
            enemy_critical_opp_ability_snapshots.append((
                read_native("wBattleMonNativeSpecies"),
                read_native("wEnemyMonNativeSpecies"),
                mem[addr("hBattleTurn")],
                mem[addr("wCurPlayerMove")],
    ...
    def observe_user_ability(_):
        ...
        if enemy_damagestats_active[0]:
            enemy_damagestats_user_ability_snapshots.append((
                read_native("wBattleMonNativeSpecies"),
                read_native("wEnemyMonNativeSpecies"),
                mem[addr("hBattleTurn")],
                mem[addr("wCurPlayerMove")],
            ...
            enemy_critical_user_ability_snapshots.append((
                read_native("wBattleMonNativeSpecies"),
                read_native("wEnemyMonNativeSpecies"),
                mem[addr("hBattleTurn")],
                mem[addr("wCurPlayerMove")],
    ...
    def observe_checkpriority(_):
        ...
        if enemy:
            ...
            # Unlike the previous checkpoint, do not stop after checkpriority.
            # Let the seventh enemy command dispatch critical for real.
            pass
        else:
    ...
    def observe_critical(_):
        enemy = len(perform_move_calls) == 2 and (mem[addr("hBattleTurn")] & 1)
        if enemy:
            enemy_critical_active[0] = True
            # Keep the previously validated player Pressure state. Remove only
            # critical-rate modifiers so the real enemy critical command takes
            # the ordinary base-rate path deterministically.
            mem[addr("wPlayerAbility")] = PRESSURE
            mem[addr("wEnemyAbility")] = 0
            mem[addr("wBattleMonItem")] = 0
            mem[addr("wEnemyMonItem")] = 0
            ...
            mem[addr("wEnemyFutureSightCount")] = 0

            enemy_critical_snapshots.append((
                read_native("wBattleMonNativeSpecies"),
                read_native("wEnemyMonNativeSpecies"),
                mem[addr("hBattleTurn")],
                mem[addr("wCurPlayerMove")],
            ...
            # Unlike the previous checkpoint, do not stop after critical.
            # Let the eighth enemy command dispatch damagestats for real.
            pass
        else:
    ...
    def observe_reset_crit(_):
        if enemy_critical_active[0]:
            enemy_reset_crit_snapshots.append((
                read_native("wBattleMonNativeSpecies"),
                read_native("wEnemyMonNativeSpecies"),
                mem[addr("hBattleTurn")],
                mem[addr("wCurPlayerMove")],
    ...
    def observe_future_sight(_):
        if enemy_damagestats_active[0]:
            enemy_damagestats_future_sight_snapshots.append((
                ...
                mem[addr("wEnemyFutureSightCount")],
            ))
        elif enemy_critical_active[0]:
            enemy_future_sight_snapshots.append((
                read_native("wBattleMonNativeSpecies"),
                read_native("wEnemyMonNativeSpecies"),
                mem[addr("hBattleTurn")],
                mem[addr("wPlayerFutureSightCount")],
    ...
    def observe_critical_affection(_):
        if affection_self_cure_active[0]:
            affection_check_calls.append((
                read_native("wBattleMonNativeSpecies"),
                read_native("wEnemyMonNativeSpecies"),
                mem[addr("hBattleTurn")],
                mem[addr("wBattleEnded")],
            ...
            enemy_critical_affection_calls.append(True)
        elif critical_active[0]:
    ...
    def observe_opponent_affection(_):
        ...
        if enemy_applydamage_active[0]:
            enemy_applydamage_affection_calls.append(True)
        elif applydamage_active[0]:
            applydamage_affection_calls.append(True)

    def observe_user_valid_item(_):
        if enemy_critical_active[0]:
            enemy_user_valid_item_calls.append(True)
        elif critical_active[0]:
    ...
    def observe_accuracy_random(_):
        ...
        if affection_self_cure_active[0]:
            affection_random_calls.append(regs.A)
        if enemy_checkhit_active[0]:
            enemy_accuracy_random_calls.append(regs.A)
        elif checkhit_active[0]:
            accuracy_random_calls.append(regs.A)
        if enemy_critical_active[0]:
            enemy_critical_random_calls.append(regs.A)
        elif critical_active[0]:
            critical_random_calls.append(regs.A)
        if enemy_damagevariation_active[0]:
            enemy_damagevariation_random_calls.append(regs.A)
        elif damagevariation_active[0]:
            damagevariation_random_calls.append(regs.A)

    def observe_damagestats(_):
        enemy = len(perform_move_calls) == 2 and (mem[addr("hBattleTurn")] & 1)
        if enemy:
            enemy_damagestats_active[0] = True
            # Earlier fixtures only need HP/level, so synthetic combat stats
            # are zero. Seed enemy Attack and player Defense immediately before
            # the real command, matching the physical Tackle user/target sides.
            mem[addr("wEnemyMonAttack"):addr("wEnemyMonAttack") + 2] = [0, 90]
            mem[addr("wBattleMonDefense"):addr("wBattleMonDefense") + 2] = [0, 80]
            mem[addr("wPlayerScreens")] = 0
            mem[addr("wEnemyScreens")] = 0
            mem[addr("wBattleWeather")] = 0
            mem[addr("wBattleMonItem")] = 0
            mem[addr("wEnemyMonItem")] = 0
            mem[addr("wPlayerFutureSightCount")] = 0
            ...
            enemy_damagestats_snapshots.append((
                read_native("wBattleMonNativeSpecies"),
                read_native("wEnemyMonNativeSpecies"),
                mem[addr("hBattleTurn")],
                mem[addr("wCurPlayerMove")],
            ...
            # Unlike the previous checkpoint, do not stop after damagestats.
            # Let the ninth enemy command dispatch damagecalc for real.
            pass
        else:
            ...
            # stats here, immediately before the real damagestats command.
    ...
    def observe_damagecalc(_):
        enemy = len(perform_move_calls) == 2 and (mem[addr("hBattleTurn")] & 1)
        if enemy:
            enemy_damagecalc_active[0] = True
            enemy_damagecalc_snapshots.append((
                read_native("wBattleMonNativeSpecies"),
                read_native("wEnemyMonNativeSpecies"),
                mem[addr("hBattleTurn")],
                mem[addr("wCurPlayerMove")],
            ...
            # Unlike the previous checkpoint, do not stop after damagecalc.
            # Let the tenth enemy command dispatch STAB for real.
            pass
        else:
    ...
    def observe_stab(_):
        ...
        if enemy:
            enemy_stab_active[0] = True
            # Pressure is not a type or damage modifier. Keep the previously
            # validated target ability intact while the enemy ability remains
            # neutral.
            mem[addr("wPlayerAbility")] = PRESSURE
            mem[addr("wEnemyAbility")] = 0
            enemy_stab_snapshots.append((
                read_native("wBattleMonNativeSpecies"),
                read_native("wEnemyMonNativeSpecies"),
                mem[addr("hBattleTurn")],
                mem[addr("wCurPlayerMove")],
            ...
            # Unlike the previous checkpoint, do not stop after STAB.
            # Let the eleventh enemy command dispatch damagevariation for real.
            pass
        else:
            stab_active[0] = True
            mem[addr("wPlayerAbility")] = 0
    ...
    def observe_stab_type_matchup(_):
        if enemy_stab_active[0]:
            enemy_stab_type_matchup_snapshots.append((
                read_native("wBattleMonNativeSpecies"),
                read_native("wEnemyMonNativeSpecies"),
                mem[addr("hBattleTurn")],
                mem[addr("wCurPlayerMove")],
    ...
    def observe_stab_weather(_):
        if enemy_stab_active[0]:
            enemy_stab_weather_snapshots.append((
                read_native("wBattleMonNativeSpecies"),
                read_native("wEnemyMonNativeSpecies"),
                mem[addr("hBattleTurn")],
                mem[addr("wBattleWeather")],
    ...
    def observe_damagevariation(_):
        enemy = len(perform_move_calls) == 2 and (mem[addr("hBattleTurn")] & 1)
        if enemy:
            enemy_damagevariation_active[0] = True
            enemy_damagevariation_snapshots.append((
                read_native("wBattleMonNativeSpecies"),
                read_native("wEnemyMonNativeSpecies"),
                mem[addr("hBattleTurn")],
                mem[addr("wCurPlayerMove")],
            ...
            # Unlike the previous checkpoint, do not install the post-read
            # stop here. Let the twelfth enemy command dispatch moveanim for
            # real; moveanim itself installs the stop for the following
            # failuretext read.
        else:
    ...
    def observe_moveanim(_):
        enemy = len(perform_move_calls) == 2 and (mem[addr("hBattleTurn")] & 1)
        if enemy:
            enemy_moveanim_active[0] = True
            enemy_moveanim_snapshots.append((
                read_native("wBattleMonNativeSpecies"),
                read_native("wEnemyMonNativeSpecies"),
                mem[addr("hBattleTurn")],
                mem[addr("wCurPlayerMove")],
            ...
            # Keep the real moveanim body untouched and continue into the
            # thirteenth enemy command, failuretext. The stop is installed
            # only when failuretext itself has actually been dispatched.
        else:
    ...
    def observe_moveanim_lowersub(_):
        if enemy_moveanim_active[0]:
            enemy_moveanim_lowersub_calls.append(True)
        elif moveanim_active[0]:
            moveanim_lowersub_calls.append(True)

    def observe_moveanim_nosub(_):
        if enemy_moveanim_active[0]:
            enemy_moveanim_nosub_calls.append(True)
        elif moveanim_active[0]:
            moveanim_nosub_calls.append(True)

    def observe_moveanim_raisesub(_):
        if enemy_moveanim_active[0]:
            enemy_moveanim_raisesub_calls.append(True)
        elif moveanim_active[0]:
            moveanim_raisesub_calls.append(True)

    def observe_moveanim_fx(_):
        if enemy_moveanim_active[0]:
            enemy_moveanim_fx_calls.append(True)
            enemy_moveanim_fx_snapshots.append((
                read_native("wBattleMonNativeSpecies"),
                read_native("wEnemyMonNativeSpecies"),
                mem[addr("hBattleTurn")],
                mem[addr("wCurPlayerMove")],
    ...
    def observe_failuretext(_):
        ...
        if len(perform_move_calls) == 2 and (mem[addr("hBattleTurn")] & 1):
            enemy_failuretext_snapshots.append((
                read_native("wBattleMonNativeSpecies"),
                read_native("wEnemyMonNativeSpecies"),
                mem[addr("hBattleTurn")],
                mem[addr("wCurPlayerMove")],
            ...
            # Continue through the real applydamage command. The controlled
            # post-read stop is installed when applydamage itself dispatches.
    ...
    def observe_applydamage(_):
        enemy = len(perform_move_calls) == 2 and (mem[addr("hBattleTurn")] & 1)
        if enemy:
            enemy_applydamage_active[0] = True
            enemy_applydamage_snapshots.append((
                read_native("wBattleMonNativeSpecies"),
                read_native("wEnemyMonNativeSpecies"),
                mem[addr("hBattleTurn")],
                mem[addr("wCurPlayerMove")],
    ...
    def observe_applydamage_item(_):
        if enemy_applydamage_active[0]:
            enemy_applydamage_item_calls.append(True)
        elif applydamage_active[0]:
    ...
    def observe_criticaltext(_):
        enemy = len(perform_move_calls) == 2 and (mem[addr("hBattleTurn")] & 1)
        if enemy:
            enemy_criticaltext_active[0] = True
            enemy_criticaltext_snapshots.append((
                read_native("wBattleMonNativeSpecies"),
                read_native("wEnemyMonNativeSpecies"),
                mem[addr("hBattleTurn")],
                mem[addr("wCurPlayerMove")],
            ...
            # Continue through the real supereffectivetext command. The
            # controlled post-read stop is installed only when
            # supereffectivetext itself dispatches.
        else:
    ...
    def observe_criticaltext_checkcrit(_):
        if enemy_criticaltext_active[0]:
            enemy_criticaltext_checkcrit_calls.append(True)
            enemy_criticaltext_checkcrit_snapshots.append((
                read_native("wBattleMonNativeSpecies"),
                read_native("wEnemyMonNativeSpecies"),
                mem[addr("hBattleTurn")],
                mem[addr("wMoveHitState")],
            ))
        elif criticaltext_active[0]:
    ...
    def observe_criticaltext_delay(_):
        if enemy_criticaltext_active[0]:
            enemy_criticaltext_delay_calls.append(True)
            enemy_criticaltext_delay_snapshots.append((
                read_native("wBattleMonNativeSpecies"),
                read_native("wEnemyMonNativeSpecies"),
                mem[addr("hBattleTurn")],
                regs.C,
    ...
    def observe_supereffectivetext(_):
        enemy = len(perform_move_calls) == 2 and (mem[addr("hBattleTurn")] & 1)
        if enemy:
            enemy_supereffectivetext_active[0] = True
            enemy_supereffectivetext_snapshots.append((
                read_native("wBattleMonNativeSpecies"),
                read_native("wEnemyMonNativeSpecies"),
                mem[addr("hBattleTurn")],
                mem[addr("wCurPlayerMove")],
            ...
            # Continue through the real postfainteffects command. The
            # controlled post-read stop is installed only when
            # postfainteffects itself dispatches.
        else:
    ...
    def observe_supereffective_textbox(_):
        if enemy_supereffectivetext_active[0]:
            enemy_supereffectivetext_text_calls.append(True)
        elif supereffectivetext_active[0]:
    ...
    def observe_supereffective_item(_):
        if enemy_supereffectivetext_active[0]:
            enemy_supereffectivetext_item_calls.append(True)
        elif supereffectivetext_active[0]:
    ...
    def observe_postfainteffects(_):
        enemy = len(perform_move_calls) == 2 and (mem[addr("hBattleTurn")] & 1)
        if enemy:
            enemy_postfainteffects_active[0] = True
            enemy_postfainteffects_snapshots.append((
                read_native("wBattleMonNativeSpecies"),
                read_native("wEnemyMonNativeSpecies"),
                mem[addr("hBattleTurn")],
                mem[addr("wCurPlayerMove")],
            ...
            # Continue through the real posthiteffects command. The
            # controlled post-read stop is installed only when
            # posthiteffects itself dispatches.
        else:
    ...
    def observe_run_faint_abilities(_):
        if enemy_postfainteffects_active[0]:
            enemy_postfaint_run_faint_abilities_calls.append(True)

    def observe_posthiteffects(_):
        enemy = len(perform_move_calls) == 2 and (mem[addr("hBattleTurn")] & 1)
        if enemy:
            enemy_posthiteffects_active[0] = True
            enemy_posthiteffects_snapshots.append((
                read_native("wBattleMonNativeSpecies"),
                read_native("wEnemyMonNativeSpecies"),
                mem[addr("hBattleTurn")],
                mem[addr("wCurPlayerMove")],
            ...
            # Continue through the original terminal $ff endmove byte. The
            # controlled stop is armed only from the enemy CheckPowerHerb
            # boundary and lands at PerformMove.end_protect after DoTurn
            # returns, before cleanup begins.
        else:
    ...
    def observe_endmove_effects(_):
        if len(perform_move_calls) == 2 and (mem[addr("hBattleTurn")] & 1):
            enemy_endmove_effect_calls.append((
                read_native("wBattleMonNativeSpecies"),
                read_native("wEnemyMonNativeSpecies"),
                mem[addr("hBattleTurn")],
                mem[addr("wCurPlayerMove")],
    ...
    def observe_endmove_throat_spray(_):
        ...
        if len(perform_move_calls) == 2 and (mem[addr("hBattleTurn")] & 1):
            enemy_endmove_throat_spray_calls.append(snapshot)
        else:
            endmove_throat_spray_calls.append(snapshot)
    ...
    def observe_endmove_power_herb(_):
        ...
        if len(perform_move_calls) == 2 and (mem[addr("hBattleTurn")] & 1):
            ...
            # helper chain, immediately before enemy PerformMove cleanup.
            enemy_cleanup_boundary_calls.append((
                ...
                mem[addr("wDamageTaken")],
                mem[addr("wDamageTaken") + 1],
                len(cleanup_tick_calls),
                len(cleanup_tilemap_calls),
                len(resolve_faints_calls),
            ))
            # Seed only state consumed by the enemy cleanup tail. The cleanup
            # itself remains real.
            cleanup_active[0] = True
            enemy_resolve_stop_armed[0] = True
            mem[addr("wPlayerSubStatus2")] |= 1 << 3  # IN_ABILITY
            mem[addr("wEnemySubStatus2")] |= 1 << 3   # IN_ABILITY
            mem[addr("wPlayerSubStatus1")] |= (1 << 2) | (1 << 5)  # Protect/Endure
    ...
    def observe_resolve_faints(_):
        if between_turn_active[0]:
            ...
            # Keep this pass out of the earlier move-cleanup observer lists.
        ...
        if enemy_resolve_stop_armed[0] and (mem[addr("hBattleTurn")] & 1):
            ...
            # This checkpoint continues through the real enemy ResolveFaints
            # body; the stop is deferred until the following DeferredSwitch
            # entry so party write-back and non-faint handling are exercised.
    ...
    def observe_deferred_switch(_):
        if resolve_active[0]:
            ...
            # Seed ability-ignorance bits immediately before each real
            # DeferredSwitch call. With wDeferredSwitch = 0 the routine must
            # return without ForceDeferredSwitch and leave this value intact
            # for the following ResetAbilityIgnorance boundary.
    ...
    def observe_reset_ability_ignorance(_):
        if resolve_active[0]:
            ...
            if enemy_resolve_stop_armed[0] and (mem[addr("hBattleTurn")] & 1):
                # Leave the enemy ResetAbilityIgnorance body and the following
                # BattleTurn.do_move wBattleEnded check/return entirely real.
                return
    ...
    def observe_post_reset(_):
        if (
            resolve_active[0]
            and enemy_resolve_stop_armed[0]
            and (mem[addr("hBattleTurn")] & 1)
        ):
            post_reset_calls.append((
                read_native("wBattleMonNativeSpecies"),
                read_native("wEnemyMonNativeSpecies"),
                mem[addr("hBattleTurn")],
                mem[addr("wDeferredSwitch")],
            ...
            # Observation only: allow the final wBattleEnded read, AND and RET
            # to execute naturally after ResetAbilityIgnorance returns.

    def observe_process_enemy_fleeing(_):
        if (
            resolve_active[0]
            and enemy_resolve_stop_armed[0]
            and (mem[addr("hBattleTurn")] & 1)
        ):
            process_enemy_fleeing_calls.append((
                read_native("wBattleMonNativeSpecies"),
                read_native("wEnemyMonNativeSpecies"),
                mem[addr("hBattleTurn")],
                mem[addr("wDeferredSwitch")],
            ...
            process_enemy_fleeing_active[0] = True

    def observe_process_enemy_can_flee(_):
        if process_enemy_fleeing_active[0]:
            process_enemy_can_flee_calls.append(True)

    def observe_process_enemy_flee_return(_):
        if (
            process_enemy_fleeing_active[0]
            and resolve_active[0]
            and enemy_resolve_stop_armed[0]
            and (mem[addr("hBattleTurn")] & 1)
        ):
            process_enemy_flee_return_calls.append((
                read_native("wBattleMonNativeSpecies"),
                read_native("wEnemyMonNativeSpecies"),
                mem[addr("hBattleTurn")],
                mem[addr("wDeferredSwitch")],
    ...
    def observe_handle_between_turn_effects(_):
        if (
            resolve_active[0]
            and enemy_resolve_stop_armed[0]
            and (mem[addr("hBattleTurn")] & 1)
        ):
            handle_between_turn_effects_calls.append((
                read_native("wBattleMonNativeSpecies"),
                read_native("wEnemyMonNativeSpecies"),
                mem[addr("hBattleTurn")],
                mem[addr("wDeferredSwitch")],
            ...
            # Separate the previous move-cleanup ResolveFaints observations
            # from the first between-turn CheckFaint/ResolveFaints pass.
            resolve_active[0] = False
            between_turn_active[0] = True

    def observe_between_turn_checkfaint(_):
        if not between_turn_active[0]:
            return
        snapshot = (
            read_native("wBattleMonNativeSpecies"),
            read_native("wEnemyMonNativeSpecies"),
        ...
        if future_sight_active[0]:
            post_future_checkfaint_calls.append(snapshot)
            post_future_checkfaint_stop_hit[0] = True
            future_sight_active[0] = False
            between_turn_active[0] = False
            regs.PC = 0xC110
        elif weather_active[0]:
            post_weather_checkfaint_calls.append(snapshot)
            # Let this second CheckFaint execute through real ResolveFaints.
    ...
    def observe_handle_weather(_):
        if between_turn_active[0]:
            ...
            weather_active[0] = True

    def observe_handle_affection_self_cure(_):
        if between_turn_active[0] and post_weather_checkfaint_calls:
            handle_affection_self_cure_calls.append((
                read_native("wBattleMonNativeSpecies"),
                read_native("wEnemyMonNativeSpecies"),
                mem[addr("hBattleTurn")],
                mem[addr("wBattleEnded")],
            ...
            affection_self_cure_active[0] = True

    def observe_handle_future_sight(_):
        if affection_self_cure_active[0] and len(affection_check_calls) == 2:
            handle_future_sight_calls.append((
                read_native("wBattleMonNativeSpecies"),
                read_native("wEnemyMonNativeSpecies"),
                mem[addr("hBattleTurn")],
                mem[addr("wBattleEnded")],
            ...
            affection_self_cure_active[0] = False
            future_sight_active[0] = True

    def observe_future_sight_do_it(_):
        if future_sight_active[0]:
            future_sight_do_it_calls.append((
                read_native("wBattleMonNativeSpecies"),
                read_native("wEnemyMonNativeSpecies"),
                mem[addr("hBattleTurn")],
                mem[addr("wBattleEnded")],
    ...
    def observe_damage_reset(_):
        if enemy_damagestats_active[0]:
            enemy_damage_reset_snapshots.append((
                read_native("wBattleMonNativeSpecies"),
                read_native("wEnemyMonNativeSpecies"),
                mem[addr("hBattleTurn")],
                mem[addr("wCurPlayerMove")],
    ...
    def observe_damage_opponent_attr(_):
        if enemy_damagestats_active[0]:
            enemy_damage_opponent_attr_snapshots.append((
                read_native("wBattleMonNativeSpecies"),
                read_native("wEnemyMonNativeSpecies"),
                mem[addr("hBattleTurn")],
                regs.HL,
    ...
    def observe_damage_screens(_):
        if enemy_damagestats_active[0]:
            enemy_damage_screen_snapshots.append((
                read_native("wBattleMonNativeSpecies"),
                read_native("wEnemyMonNativeSpecies"),
                mem[addr("hBattleTurn")],
                mem[addr("wPlayerScreens")],
                mem[addr("wEnemyScreens")],
            ))
        elif damagestats_active[0]:
    ...
    def observe_true_user_party_attr(_):
        if enemy_damagestats_active[0]:
            enemy_true_user_party_attr_snapshots.append((
                read_native("wBattleMonNativeSpecies"),
                read_native("wEnemyMonNativeSpecies"),
                mem[addr("hBattleTurn")],
                regs.A,
    ...
    def setup_case(native, species, form):
        ...
        post_future_checkfaint_stop_hit[0] = False
        process_enemy_fleeing_active[0] = False
        between_turn_active[0] = False
        weather_active[0] = False
        affection_self_cure_active[0] = False
        ...
        for calls in (
            ...
            user_valid_item_calls,
            enemy_critical_snapshots, enemy_reset_crit_snapshots,
            enemy_critical_opp_ability_snapshots,
            enemy_future_sight_snapshots,
            enemy_user_item_after_unnerve_snapshots,
            enemy_user_item_snapshots,
            enemy_critical_user_ability_snapshots,
            enemy_critical_affection_calls, enemy_critical_random_calls,
            enemy_user_valid_item_calls, enemy_critical_result_snapshots,
            damagestats_calls, damagestats_snapshots,
            ...
            damagestats_result_snapshots,
            enemy_damagestats_snapshots, enemy_damage_reset_snapshots,
            enemy_damage_opponent_attr_snapshots,
            enemy_damage_user_attr_snapshots,
            enemy_damagestats_future_sight_snapshots,
            enemy_damage_screen_snapshots,
            ...
            enemy_damagestats_user_ability_snapshots,
            enemy_damagestats_result_snapshots,
            enemy_damagecalc_snapshots, enemy_damagecalc_result_snapshots,
            enemy_stab_snapshots, enemy_stab_type_matchup_snapshots,
            enemy_stab_weather_snapshots, enemy_stab_result_snapshots,
            enemy_damagevariation_snapshots,
            enemy_damagevariation_random_calls,
            enemy_damagevariation_result_snapshots,
            enemy_moveanim_snapshots, enemy_moveanim_lowersub_calls,
            enemy_moveanim_nosub_calls, enemy_moveanim_raisesub_calls,
            enemy_moveanim_fx_calls, enemy_moveanim_fx_snapshots,
            enemy_moveanim_result_snapshots,
            enemy_failuretext_snapshots, enemy_failuretext_result_snapshots,
            ...
            applydamage_result_snapshots,
            enemy_applydamage_snapshots,
            enemy_applydamage_affection_calls, enemy_applydamage_item_calls,
            enemy_applydamage_ability_calls, enemy_applydamage_reset_subhit_calls,
            enemy_applydamage_check_sub_calls, enemy_applydamage_take_damage_calls,
            enemy_applydamage_deal_damage_calls,
            enemy_applydamage_subtract_hp_calls, enemy_applydamage_hud_calls,
            enemy_applydamage_refresh_huds_calls,
            enemy_applydamage_result_snapshots,
            ...
            post_reset_calls, process_enemy_fleeing_calls,
            process_enemy_flee_return_calls, handle_between_turn_effects_calls,
            between_turn_checkfaint_calls, between_turn_resolve_faints_calls,
            handle_weather_calls, post_weather_checkfaint_calls,
            handle_affection_self_cure_calls, affection_check_calls,
            affection_random_calls, handle_future_sight_calls,
            future_sight_do_it_calls, post_future_checkfaint_calls,
            weather_deep_calls, process_enemy_can_flee_calls,
    ...
    try:
        ...
        hook("RunFaintAbilities", observe_run_faint_abilities)
        ...
        hook("HandleFutureSight.do_it", observe_future_sight_do_it)
        ...
        for native, species, form in cases:
            ...
            invoke(
                ...
                stop_flag=post_future_checkfaint_stop_hit
            )

            assert post_future_checkfaint_stop_hit[0], (
                "HandleFutureSight did not complete both zero-counter passes "
                "and reach the following CheckFaint",
                context,
            )
            ...
            assert read_script_calls == [True] * 38, (
                "expected the 19 validated player reads plus nineteen real enemy "
                "script-byte reads",
            ...
            assert enemy_read_snapshots == [
                ...
                (
                    25, native, 1, 45, 33, 0, 0, 0,
                    read_script_snapshots[26][5],
                    read_script_snapshots[26][6],
                ),
                (
                    25, native, 1, 45, 33, 0, 0, 0,
                    read_script_snapshots[27][5],
                    read_script_snapshots[27][6],
                ),
                (
                    25, native, 1, 45, 33, 0, 0, 0,
                    read_script_snapshots[28][5],
                    read_script_snapshots[28][6],
                ),
                (
                    25, native, 1, 45, 33, 0, 0, 0,
                    read_script_snapshots[29][5],
                    read_script_snapshots[29][6],
                ),
                (
                    25, native, 1, 45, 33, 0, 0, 0,
                    read_script_snapshots[30][5],
                ...
                (
                    25, native, 1, 45, 33, 0, 0, 0x10,
                    read_script_snapshots[32][5],
                    read_script_snapshots[32][6],
                ),
                (
                    25, native, 1, 45, 33, 0, 0, 0x10,
                    read_script_snapshots[33][5],
                    read_script_snapshots[33][6],
                ),
                (
                    25, native, 1, 45, 33, 0, 0, 0x10,
                    read_script_snapshots[34][5],
                    read_script_snapshots[34][6],
                ),
                (
                    25, native, 1, 45, 33, 0, 0, 0x10,
                    read_script_snapshots[35][5],
                    read_script_snapshots[35][6],
                ),
                (
                    25, native, 1, 45, 33, 0, 0, 0x10,
                    read_script_snapshots[36][5],
                    read_script_snapshots[36][6],
                ),
                (
                    25, native, 1, 45, 33, 0, 0, 0x10,
                    read_script_snapshots[37][5],
                    read_script_snapshots[37][6],
                ),
            ], (
            ...
            assert read_script_snapshots[26][5:] == advance_script_pointer(
                read_script_snapshots[19], 7
            ), (
                "enemy critical did not return to the eighth script read at "
                "exactly the damagestats command byte",
                context, read_script_snapshots[19:27]
            )
            assert read_script_snapshots[27][5:] == advance_script_pointer(
                read_script_snapshots[19], 8
            ), (
                "enemy damagestats did not return to the ninth script read at "
                "exactly the damagecalc command byte",
                context, read_script_snapshots[19:28]
            )
            assert read_script_snapshots[28][5:] == advance_script_pointer(
                read_script_snapshots[19], 9
            ), (
                "enemy damagecalc did not return to the tenth script read at "
                "exactly the STAB command byte",
                context, read_script_snapshots[19:29]
            )
            assert read_script_snapshots[29][5:] == advance_script_pointer(
                read_script_snapshots[19], 10
            ), (
                "enemy STAB did not return to the eleventh script read at "
                "exactly the damagevariation command byte",
                context, read_script_snapshots[19:30]
            )
            ...
            assert enemy_critical_snapshots == [
                (
                    25, native, 1, 45, 33, 0, 0,
                    34, 34, 33, 33, PRESSURE, 0,
                    *advance_script_pointer(read_script_snapshots[19], 7),
                    0,
                ),
            ], (
                "enemy critical entry lost native IDs, Tackle/PP, Pressure, "
                "non-critical state, script pointer, or unended battle",
                context, enemy_critical_snapshots
            )
            assert enemy_reset_crit_snapshots == [
                (25, native, 1, 45, 33, 0),
            ], (
                "enemy critical did not enter real ResetCrit with preserved "
                "native IDs/enemy Tackle state",
                context, enemy_reset_crit_snapshots
            )
            assert len(enemy_critical_opp_ability_snapshots) == 2, (
                "enemy critical path did not run both anti-crit and Unnerve "
                "opponent ability checks",
                context, enemy_critical_opp_ability_snapshots
            )
            assert all(
                snap[0:2] == (25, native)
                and snap[2] == 1
                and snap[3:5] == (45, 33)
                and snap[5:] == (PRESSURE, 0)
                for snap in enemy_critical_opp_ability_snapshots
            ), (
                "enemy critical opponent ability checks changed native/move/"
                "ability state",
                context, enemy_critical_opp_ability_snapshots
            )
            assert len(enemy_future_sight_snapshots) == 4, (
                "enemy critical path Future Sight user-check count",
                context, enemy_future_sight_snapshots
            )
            assert all(
                snap == (25, native, 1, 0, 0)
                for snap in enemy_future_sight_snapshots
            ), (
                "enemy critical Future Sight checks changed native/turn/count "
                "state",
                context, enemy_future_sight_snapshots
            )
            assert enemy_user_item_after_unnerve_snapshots == [
                (25, native, 1, 0, 0),
            ], (
                "enemy critical skipped or changed held-item-after-Unnerve "
                "state",
                context, enemy_user_item_after_unnerve_snapshots
            )
            assert len(enemy_user_item_snapshots) == 2, (
                "enemy critical did not perform both held-item reads",
                context, enemy_user_item_snapshots
            )
            assert all(
                snap == (25, native, 1, 0, 0)
                for snap in enemy_user_item_snapshots
            ), (
                "enemy critical item reads changed native/turn/item state",
                context, enemy_user_item_snapshots
            )
            assert enemy_user_valid_item_calls == [], (
                "zero enemy held item unexpectedly entered species-valid item "
                "path", context, enemy_user_valid_item_calls
            )
            assert enemy_critical_user_ability_snapshots == [
                (25, native, 1, 45, 33, PRESSURE, 0),
            ], (
                "enemy critical skipped Super Luck user ability check or "
                "changed native/move/ability state",
                context, enemy_critical_user_ability_snapshots
            )
            assert enemy_critical_affection_calls == [True], (
                "enemy critical path skipped affection check",
                context, enemy_critical_affection_calls
            )
            assert enemy_critical_random_calls == [24], (
                "enemy critical path did not request the base 0-23 roll",
                context, enemy_critical_random_calls
            )
            assert enemy_critical_result_snapshots == [
                (
                    25, native, 1, 45, 33, 0,
                    34, 34, 33, 33, PRESSURE, 0, 0, 0,
                    *advance_script_pointer(read_script_snapshots[19], 7),
                ),
            ], (
                "enemy critical did not return non-critical with preserved "
                "native IDs, Tackle/PP, Pressure, hit state, and pre-"
                "damagestats-read pointer",
                context, enemy_critical_result_snapshots
            )
            assert enemy_damagestats_snapshots == [
                (
                    25, native, 1, 45, 33, 0, 0,
                    34, 34, 33, 33, PRESSURE, 0,
                    *advance_script_pointer(read_script_snapshots[19], 8),
                    0,
                ),
            ], (
                "enemy damagestats entry lost native IDs, Tackle/PP, Pressure, "
                "hit state, script pointer, or unended battle",
                context, enemy_damagestats_snapshots
            )
            assert enemy_damage_reset_snapshots == [
                (25, native, 1, 45, 33),
            ], (
                "enemy damagestats did not enter real ResetDamage with "
                "preserved native/enemy move state",
                context, enemy_damage_reset_snapshots
            )
            assert enemy_damage_opponent_attr_snapshots, (
                "enemy physical damagestats did not read target Defense",
                context, enemy_damage_opponent_attr_snapshots
            )
            assert (25, native, 1, addr("wBattleMonDefense")) in (
                enemy_damage_opponent_attr_snapshots
            ), (
                "enemy physical damagestats never selected player Defense",
                context, enemy_damage_opponent_attr_snapshots
            )
            assert all(
                snap[:2] == (25, native)
                for snap in enemy_damage_opponent_attr_snapshots
            ), (
                "native identity changed during enemy opponent-stat reads",
                context, enemy_damage_opponent_attr_snapshots
            )
            assert enemy_damage_user_attr_snapshots, (
                "enemy physical damagestats did not read user Attack",
                context, enemy_damage_user_attr_snapshots
            )
            assert any(
                snap[0:2] == (25, native)
                and snap[3] == addr("wBattleMonAttack")
                for snap in enemy_damage_user_attr_snapshots
            ), (
                "enemy physical damagestats never selected shared Attack "
                "attribute through enemy-user perspective",
                context, enemy_damage_user_attr_snapshots
            )
            assert all(
                snap[:2] == (25, native)
                ...
                "native identity changed during enemy user-stat reads",
                context, enemy_damage_user_attr_snapshots
            )
            assert enemy_damagestats_future_sight_snapshots, (
                "enemy damagestats skipped Future Sight user resolution",
                context, enemy_damagestats_future_sight_snapshots
            )
            assert all(
                snap[0:2] == (25, native)
                and snap[3:] == (0, 0)
                for snap in enemy_damagestats_future_sight_snapshots
            ), (
                "enemy damagestats Future Sight checks changed identity/counts",
                context, enemy_damagestats_future_sight_snapshots
            )
            assert any(
                snap[2] == 1 for snap in enemy_damagestats_future_sight_snapshots
            ), (
                "enemy damagestats never resolved enemy-side Future Sight user",
                context, enemy_damagestats_future_sight_snapshots
            )
            assert enemy_damage_screen_snapshots == [
                (25, native, 1, 0, 0),
            ], (
                "enemy physical damagestats skipped neutral player-screen "
                "handling or changed native identity",
                context, enemy_damage_screen_snapshots
            )
            assert len(enemy_true_user_party_attr_snapshots) >= 3, (
                "enemy damagestats did not perform item and level party reads",
                context, enemy_true_user_party_attr_snapshots
            )
            assert all(
                snap[0:2] == (25, native)
                for snap in enemy_true_user_party_attr_snapshots
            ), (
                "native identity changed during enemy true-user party reads",
                context, enemy_true_user_party_attr_snapshots
            )
            assert enemy_damagestats_user_ability_snapshots, (
                "enemy screen handling skipped real user ability lookup",
                context, enemy_damagestats_user_ability_snapshots
            )
            assert all(
                snap[0:2] == (25, native)
                and snap[3:5] == (45, 33)
                for snap in enemy_damagestats_user_ability_snapshots
            ), (
                "enemy damagestats ability lookup changed native/move state",
                context, enemy_damagestats_user_ability_snapshots
            )
            assert enemy_damagestats_result_snapshots, (
                "missing enemy damagestats result at ninth enemy script read",
                context, enemy_damagestats_result_snapshots
            )
            enemy_damage_result = enemy_damagestats_result_snapshots[0]
            assert enemy_damage_result[:13] == (
                25, native, 1, 45, 33, 0,
                34, 34, 33, 33, PRESSURE, 0, 0,
            ), (
                "enemy damagestats result changed native/move/PP/ability/hit "
                "state",
                context, enemy_damage_result
            )
            assert enemy_damage_result[13] > 0 and enemy_damage_result[14] > 0, (
                "enemy damagestats returned zero Attack/Defense",
                context, enemy_damage_result
            )
            assert enemy_damage_result[15:17] == (40, 30), (
                "enemy damagestats returned wrong Tackle power/enemy level",
                context, enemy_damage_result
            )
            assert enemy_damage_result[17:] == advance_script_pointer(
                read_script_snapshots[19], 8
            ), (
                "enemy damagestats result captured wrong pre-damagecalc pointer",
                context, enemy_damage_result
            )
            assert enemy_damagecalc_snapshots == [
                (
                    25, native, 1, 45, 33, 0,
                    34, 34, 33, 33, PRESSURE, 0, 0,
                    enemy_damage_result[13], enemy_damage_result[14], 40, 30,
                    0, 0,
                    *advance_script_pointer(read_script_snapshots[19], 9),
                ),
            ], (
                "enemy damagecalc entry lost native IDs, Tackle/PP, Pressure, "
                "damagestats inputs, reset damage state, or script pointer",
                context, enemy_damagecalc_snapshots, enemy_damage_result
            )
            assert enemy_damagecalc_result_snapshots == [
                (
                    25, native, 1, 45, 33, 0,
                    34, 34, 33, 33, PRESSURE, 0, 0, 0, 14,
                    *advance_script_pointer(read_script_snapshots[19], 9),
                ),
            ], (
                "real enemy base damage formula did not produce expected "
                "neutral Tackle damage or changed native/enemy move state",
                context, enemy_damagecalc_result_snapshots
            )
            assert enemy_stab_snapshots == [
                (
                    25, native, 1, 45, 33, 0,
                    34, 34, 33, 33, PRESSURE, 0, 0, 0, 14,
                    *advance_script_pointer(read_script_snapshots[19], 10),
                ),
            ], (
                "enemy STAB entry lost native IDs, Tackle/PP, Pressure, base "
                "damage, or script pointer",
                context, enemy_stab_snapshots
            )
            assert enemy_stab_type_matchup_snapshots == [
                (25, native, 1, 45, 33, PRESSURE, 0, 0, 14),
            ], (
                "enemy STAB skipped real type-matchup handling or changed "
                "native/ability/base-damage state",
            ...
            assert enemy_stab_result_snapshots == [
                ...
                "real enemy STAB/type handling did not produce neutral 1.5x "
                "Tackle damage 21 or changed native/enemy move state",
                context, enemy_stab_result_snapshots
            )
            assert enemy_damagevariation_snapshots == [
                (
                    25, native, 1, 45, 33, 0,
                    34, 34, 33, 33, PRESSURE, 0, 0, 0, 21,
                    *advance_script_pointer(read_script_snapshots[19], 11),
                ),
            ], (
                "enemy damagevariation entry lost native IDs, Tackle/PP, "
                "Pressure, STAB damage, or script pointer",
                context, enemy_damagevariation_snapshots
            )
            assert enemy_damagevariation_random_calls == [16], (
                "enemy damagevariation did not request its real 0-15 roll",
                context, enemy_damagevariation_random_calls
            )
            assert enemy_damagevariation_result_snapshots == [
                (
                    25, native, 1, 45, 33, 0,
                ...
                "real enemy 85% damage variation did not produce 17 damage or "
                "changed native/enemy move state",
                context, enemy_damagevariation_result_snapshots
            )
            assert read_script_snapshots[30][5:] == advance_script_pointer(
                read_script_snapshots[19], 11
            ), (
                "enemy damagevariation did not return to the twelfth script "
                "read at exactly the moveanim command byte",
                context, read_script_snapshots[29:31]
            )
            assert enemy_moveanim_snapshots == [
                (
                    25, native, 1, 45, 33, 0,
                    34, 34, 33, 33, PRESSURE, 0, 0, 0, 17,
                    *advance_script_pointer(read_script_snapshots[19], 12),
                ),
            ], (
                "enemy moveanim entry lost native IDs, Tackle/PP, Pressure, "
                "varied damage, or script pointer",
                context, enemy_moveanim_snapshots
            )
            assert enemy_moveanim_lowersub_calls == [True], (
                "enemy moveanim skipped lowersub control flow",
                context, enemy_moveanim_lowersub_calls
            )
            assert enemy_moveanim_nosub_calls == [True], (
                "enemy moveanim skipped moveanimnosub control flow",
                context, enemy_moveanim_nosub_calls
            )
            assert enemy_moveanim_raisesub_calls == [True], (
                "enemy moveanim skipped raisesub control flow",
                context, enemy_moveanim_raisesub_calls
            )
            assert enemy_moveanim_fx_calls == [True], (
                "enemy moveanim did not reach PlayFXAnimID",
                context, enemy_moveanim_fx_calls
            )
            assert enemy_moveanim_fx_snapshots == [
                (25, native, 1, 45, 33, 33),
            ], (
                "enemy moveanim requested the wrong animation or changed "
                "native/enemy move state at PlayFXAnimID",
                context, enemy_moveanim_fx_snapshots
            )
            assert enemy_moveanim_result_snapshots == [
                (
                    25, native, 1, 45, 33, 0,
                    34, 34, 33, 33, PRESSURE, 0, 0, 0, 17, 0,
                    *advance_script_pointer(read_script_snapshots[19], 12),
                ),
            ], (
                "real enemy moveanim changed native/enemy move/PP/Pressure/"
                "damage state or left an unexpected animation parameter",
                context, enemy_moveanim_result_snapshots
            )
            assert read_script_snapshots[31][5:] == advance_script_pointer(
                read_script_snapshots[19], 12
            ), (
                "enemy moveanim did not return to the thirteenth script read "
                "at exactly the failuretext command byte",
                context, read_script_snapshots[30:32]
            )
            assert enemy_failuretext_snapshots == [
                ...
                "enemy failuretext entry lost native IDs, Tackle/PP, Pressure, "
                "successful-hit state, varied damage, or script pointer",
                context, enemy_failuretext_snapshots
            )
            assert enemy_failuretext_result_snapshots == [
                ...
                "real enemy failuretext no-failure return changed native/enemy "
                "move/PP/Pressure/damage state",
                context, enemy_failuretext_result_snapshots
            )
            assert read_script_snapshots[32][5:] == advance_script_pointer(
                read_script_snapshots[19], 13
            ), (
                "enemy failuretext did not return to the fourteenth script read "
                "at exactly the applydamage command byte",
                context, read_script_snapshots[31:33]
            )
            assert enemy_applydamage_snapshots == [
                (
                    25, native, 1, 45, 33, 0,
                    34, 34, 33, 33, PRESSURE, 0, 0, 0, 0, 17,
                    0, 100, 0, 0,
                    *advance_script_pointer(read_script_snapshots[19], 14),
                ),
            ], (
                "enemy applydamage entry lost native IDs, Tackle/PP, Pressure, "
                "successful-hit state, damage, player HP, or script pointer",
                context, enemy_applydamage_snapshots
            )
            assert enemy_applydamage_reset_subhit_calls == [True], (
                "enemy applydamage skipped ResetSubHit",
                context, enemy_applydamage_reset_subhit_calls
            )
            assert enemy_applydamage_check_sub_calls, (
                "enemy applydamage skipped substitute check",
                context, enemy_applydamage_check_sub_calls
            )
            assert enemy_applydamage_affection_calls == [True], (
                "enemy applydamage skipped affection endure check",
                context, enemy_applydamage_affection_calls
            )
            assert enemy_applydamage_item_calls == [True], (
                "enemy applydamage skipped player-target held-item check",
                context, enemy_applydamage_item_calls
            )
            assert enemy_applydamage_ability_calls, (
                "enemy applydamage skipped player-target ability checks",
                context, enemy_applydamage_ability_calls
            )
            assert enemy_applydamage_take_damage_calls == [True], (
                "enemy applydamage skipped TakeDamage",
                context, enemy_applydamage_take_damage_calls
            )
            assert enemy_applydamage_deal_damage_calls == [True], (
                "enemy applydamage skipped DealDamageToOpponent",
                context, enemy_applydamage_deal_damage_calls
            )
            assert enemy_applydamage_subtract_hp_calls == [True], (
                "enemy applydamage skipped SubtractHPFromUser",
                context, enemy_applydamage_subtract_hp_calls
            )
            assert enemy_applydamage_hud_calls == [True], (
                "enemy HP subtraction did not reach HP-bar update boundary",
                context, enemy_applydamage_hud_calls
            )
            assert enemy_applydamage_refresh_huds_calls == [True], (
                "enemy TakeDamage did not reach final HUD refresh boundary",
                context, enemy_applydamage_refresh_huds_calls
            )
            assert enemy_applydamage_result_snapshots == [
                (
                    25, native, 1, 45, 33, 0,
                    34, 34, 33, 33, PRESSURE, 0, 0, 0, 0, 17,
                    0, 83, 0, 17,
                ...
                "real enemy applydamage did not subtract 17 HP from the player "
                "or preserve native/enemy move/PP/Pressure/damage state",
                context, enemy_applydamage_result_snapshots
            )
            assert read_script_snapshots[33][5:] == advance_script_pointer(
                read_script_snapshots[19], 14
            ), (
                "enemy applydamage did not return to the fifteenth script read "
                "at exactly the criticaltext command byte",
                context, read_script_snapshots[32:34]
            )
            assert enemy_criticaltext_snapshots == [
                (
                    25, native, 1, 45, 33, 0,
                    34, 34, 33, 33, PRESSURE, 0, 0, 0,
                    0, 83, 0, 17,
                    *advance_script_pointer(read_script_snapshots[19], 15),
                ),
            ], (
                "enemy criticaltext entry lost native IDs, Tackle/PP, Pressure, "
                "non-critical state, player HP, damage bookkeeping, or pointer",
                context, enemy_criticaltext_snapshots
            )
            assert enemy_criticaltext_checkcrit_calls == [True], (
                "enemy criticaltext skipped CheckCrit",
                context, enemy_criticaltext_checkcrit_calls
            )
            assert enemy_criticaltext_checkcrit_snapshots == [
                (25, native, 1, 0),
            ], (
                "enemy CheckCrit saw changed native identity or hit state",
                context, enemy_criticaltext_checkcrit_snapshots
            )
            assert enemy_criticaltext_delay_calls == [True], (
                "enemy non-critical criticaltext skipped its wait",
                context, enemy_criticaltext_delay_calls
            )
            assert enemy_criticaltext_delay_snapshots == [
                (25, native, 1, 20),
            ], (
                "enemy criticaltext did not request DelayFrames(20)",
                context, enemy_criticaltext_delay_snapshots
            )
            assert enemy_criticaltext_result_snapshots == [
                (
                    25, native, 1, 45, 33, 0,
                    34, 34, 33, 33, PRESSURE, 0, 0, 0,
                    0, 83, 0, 17,
                ...
                "real enemy criticaltext changed native/enemy move/PP/Pressure/"
                "hit state, player HP, or damage bookkeeping",
                context, enemy_criticaltext_result_snapshots
            )
            assert read_script_snapshots[34][5:] == advance_script_pointer(
                read_script_snapshots[19], 15
            ), (
                "enemy criticaltext did not return to the sixteenth script read "
                "at exactly the supereffectivetext command byte",
                context, read_script_snapshots[33:35]
            )
            assert enemy_supereffectivetext_snapshots == [
                (
                    25, native, 1, 45, 33, 0,
                    34, 34, 33, 33, PRESSURE, 0, 0x10,
                    0, 83, 0, 17,
                    *advance_script_pointer(read_script_snapshots[19], 16),
                ),
            ], (
                "enemy supereffectivetext entry lost native IDs, Tackle/PP, "
                "Pressure, neutral type modifier, player HP, damage, or pointer",
                context, enemy_supereffectivetext_snapshots
            )
            assert enemy_supereffectivetext_text_calls == [], (
                "neutral enemy supereffectivetext printed effectiveness text",
                context, enemy_supereffectivetext_text_calls
            )
            assert enemy_supereffectivetext_item_calls == [], (
                "neutral enemy supereffectivetext entered Weakness Policy handling",
                context, enemy_supereffectivetext_item_calls
            )
            assert enemy_supereffectivetext_result_snapshots == [
                (
                    25, native, 1, 45, 33, 0,
                    34, 34, 33, 33, PRESSURE, 0, 0x10,
                    0, 83, 0, 17,
                ...
                "real enemy supereffectivetext changed native/enemy move/PP/"
                "Pressure/type modifier, player HP, or damage bookkeeping",
                context, enemy_supereffectivetext_result_snapshots
            )
            assert read_script_snapshots[35][5:] == advance_script_pointer(
                read_script_snapshots[19], 16
            ), (
                "enemy supereffectivetext did not return to the seventeenth "
                "script read at exactly the postfainteffects command byte",
                context, read_script_snapshots[34:36]
            )
            assert enemy_postfainteffects_snapshots == [
                (
                    25, native, 1, 45, 33, 0,
                    34, 34, 33, 33, PRESSURE,
                    0, 83, 0, 17,
                    *advance_script_pointer(read_script_snapshots[19], 17),
                ),
            ], (
                "enemy postfainteffects entry lost native IDs, Tackle/PP, "
                "Pressure, player HP, damage bookkeeping, or pointer",
                context, enemy_postfainteffects_snapshots
            )
            assert enemy_postfaint_fainted_calls == [
                (25, native, 1, 0, 83, 0, 83),
            ], (
                "enemy postfainteffects did not check the living player target "
                "with the validated HP state",
                context, enemy_postfaint_fainted_calls
            )
            assert enemy_postfaint_run_faint_abilities_calls == [], (
                "living player target incorrectly entered RunFaintAbilities",
                context, enemy_postfaint_run_faint_abilities_calls
            )
            assert enemy_postfainteffects_result_snapshots == [
                (
                    25, native, 1, 45, 33, 0,
                    34, 34, 33, 33, PRESSURE,
                    0, 83, 0, 17,
                ...
                "real enemy postfainteffects changed native/enemy move/PP/"
                "Pressure, player HP, or damage bookkeeping",
                context, enemy_postfainteffects_result_snapshots
            )
            assert read_script_snapshots[36][5:] == advance_script_pointer(
                read_script_snapshots[19], 17
            ), (
                "enemy postfainteffects did not return to the eighteenth "
                "script read at exactly the posthiteffects command byte",
                context, read_script_snapshots[35:37]
            )
            assert enemy_posthiteffects_snapshots == [
                (
                    25, native, 1, 45, 33, 0,
                    34, 34, 33, 33, PRESSURE,
                    0, 83, 0, 17, 0,
                    *advance_script_pointer(read_script_snapshots[19], 18),
                ),
            ], (
                "enemy posthiteffects entry lost native IDs, Tackle/PP, "
                "Pressure, player HP, damage/hit state, or pointer",
                context, enemy_posthiteffects_snapshots
            )
            assert enemy_posthiteffects_result_snapshots == [
                (
                    25, native, 1, 45, 33, 0,
                    34, 34, 33, 33, PRESSURE,
                    0, 83, 0, 17, 0,
                ...
                "real enemy posthiteffects changed native/enemy move/PP/"
                "Pressure, player HP, damage bookkeeping, or hit state",
                context, enemy_posthiteffects_result_snapshots
            )
            assert read_script_snapshots[37][5:] == advance_script_pointer(
                read_script_snapshots[19], 18
            ), (
                "enemy posthiteffects did not return to the nineteenth "
                "script read at exactly the endmove command byte",
                context, read_script_snapshots[36:38]
            )
            assert enemy_endmove_effect_calls == [
                (
                    25, native, 1, 45, 33,
                    34, 34, 33, 33, PRESSURE,
                    0, 83, 0, 83, 0, 17,
                ...
                "enemy terminal endmove did not enter CheckEndMoveEffects with "
                "the validated native/move/PP/Pressure/HP/damage state",
                context, enemy_endmove_effect_calls
            )
            assert enemy_endmove_throat_spray_calls == [(25, native, 1)], (
                "enemy terminal endmove did not continue to CheckThroatSpray",
                context, enemy_endmove_throat_spray_calls
            )
            assert enemy_endmove_power_herb_calls == [(25, native, 1)], (
                "enemy terminal endmove did not continue to CheckPowerHerb",
                context, enemy_endmove_power_herb_calls
            )
            assert enemy_cleanup_boundary_calls == [
                (
                    25, native, 1, 45, 33,
                    34, 34, 33, 33, PRESSURE,
                    0, 83, 0, 83, 0, 17,
            ...
            # The player-side snapshots above prove moveanim preserved the
            # varied 17 damage. Enemy damagestats resets that state, damagecalc
            # computes base damage 14, STAB raises it to 21, and the newly-real
            # enemy damagevariation command applies the deterministic 85% floor.
            assert bytes(
                mem[addr("wCurDamage"):addr("wCurDamage") + 2]
            ) == bytes([0, 17]), (
                "enemy damagevariation did not leave expected 17 damage", context,
            ...
            assert failuretext_calls == [True, True], (
                "player/enemy real failuretext dispatch count", context,
                failuretext_calls
            )
            ...
            assert len(read_script_calls) == 38, (
                "expected the validated 19 player reads plus nineteen enemy reads",
            ...
            assert cleanup_tick_calls == [
                (25, native, 0, 0, 0, 0, (1 << 2) | (1 << 5), 5, 4, 7, 6),
                (25, native, 1, 0, 0, (1 << 2) | (1 << 5), 0, 5, 4, 7, 6),
            ], (
            ...
            assert resolve_faints_calls == [
                ...
                (25, native, 1, 0, 0, 0, 5, 4, 7, 6, 0, 83, 0, 17),
            ], (
                "both player and enemy cleanup paths did not enter real "
                "ResolveFaints with preserved native identity, HP, damage, "
                "and side-specific cleanup state",
            ...
            assert resolve_player_writeback_calls == [
                ...
                "real player party write-back boundaries changed identity/HP/PP",
            ...
            assert resolve_enemy_writeback_calls == [
                ...
                "real enemy party write-back boundaries changed identity/layout",
            ...
            assert resolve_enemy_fainted_calls == [
                ...
                "opponent checks on both move sides",
            ...
            assert deferred_switch_calls == [
                ...
                "enemy ResolveFaints did not return to DeferredSwitch with "
                "preserved native/battle state",
            ...
            assert reset_ability_ignorance_calls == [
                ...
                (25, native, 1, 0, 0x55, 0),
            ], (
                "real enemy no-op DeferredSwitch did not reach "
                "ResetAbilityIgnorance with the seeded move-state sentinel",
                context, reset_ability_ignorance_calls
            )
            assert post_reset_calls == [
                (25, native, 1, 0, 0x11, 0),
            ], (
                "real enemy ResetAbilityIgnorance did not clear only the two "
                "ability-ignorance bits before the final wBattleEnded check",
                context, post_reset_calls
            )
            assert process_enemy_fleeing_calls == [
                (25, native, 1, 0, 0x11, 0, 0),
            ], (
                "BattleTurn.do_move did not reach ProcessEnemyFleeing with "
                "wEnemyFleeing=0 and preserved state",
                context, process_enemy_fleeing_calls
            )
            assert process_enemy_can_flee_calls == [], (
                "wEnemyFleeing=0 unexpectedly entered EnemyCanFlee",
                context, process_enemy_can_flee_calls
            )
            assert process_enemy_flee_return_calls == [
                (25, native, 1, 0, 0x11, 0, 0),
            ], (
                "ProcessEnemyFleeing did not take its immediate zero-flag return "
                "to the post-call wBattleEnded boundary",
                context, process_enemy_flee_return_calls
            )
            assert handle_between_turn_effects_calls == [
                (25, native, 1, 0, 0x11, 0, 0),
            ], (
                "zero wBattleEnded did not fall through RET NZ to the "
                "HandleBetweenTurnEffects entry with preserved state",
                context, handle_between_turn_effects_calls
            )
            ...
            assert between_turn_resolve_faints_calls == [
                (25, native, 1, 0, 0, 0, 83, 0, 83, 0, 17, 0x11),
                (25, native, 1, 0, 0, 0, 83, 0, 83, 0, 17, 0x11),
            ], (
                "both between-turn CheckFaint calls did not execute real "
                "ResolveFaints with preserved HP/damage state",
                context, between_turn_resolve_faints_calls
            )
            assert handle_weather_calls == [
                (25, native, 1, 0, 0, 0, 83, 0, 83, 0x11, 0),
            ], (
                "initial CheckFaint did not return carry clear through RET C "
                "to HandleWeather with WEATHER_NONE",
                context, handle_weather_calls
            )
            assert weather_deep_calls == [], (
                "HandleWeather WEATHER_NONE path unexpectedly entered deeper "
                "weather processing",
                context, weather_deep_calls
            )
            assert post_weather_checkfaint_calls == [
                (25, native, 1, 0, 0, 0, 83, 0, 83, 0x11, 0),
            ], (
                "HandleWeather did not take its immediate RET Z and reach the "
                "following CheckFaint with preserved state",
                context, post_weather_checkfaint_calls
            )
            assert handle_affection_self_cure_calls == [
                (25, native, 1, 0, 0, 0, 83, 0, 83, 0, 17, 0x11, 0),
            ], (
                "second CheckFaint did not return carry clear through RET C to "
                "HandleAffectionSelfCure with preserved state",
                context, handle_affection_self_cure_calls
            )
            assert len(affection_check_calls) == 2, (
                "HandleAffectionSelfCure did not run CheckAffection for both sides",
                context, affection_check_calls
            )
            assert {snap[2] for snap in affection_check_calls} == {0, 1}, (
                "HandleAffectionSelfCure did not visit one player and one enemy turn",
                context, affection_check_calls
            )
            for snap in affection_check_calls:
                assert snap[0:2] == (25, native), (
                    "affection pass changed native identities", context, snap
                )
                assert snap[3:] == (
                    0, 0, 0, 83, 0, 83, 0, 17, 0x11, 0
                ), (
                    "affection pass changed battle/HP/damage/weather state",
                    context, snap
                )
            assert affection_random_calls == [], (
                "below-threshold affection unexpectedly entered cure RNG",
                context, affection_random_calls
            )
            assert len(handle_future_sight_calls) == 1, (
                "HandleAffectionSelfCure did not fall through to HandleFutureSight",
                context, handle_future_sight_calls
            )
            future = handle_future_sight_calls[0]
            assert future[0:2] == (25, native), (
                "native identities changed before HandleFutureSight", context, future
            )
            assert future[2] == affection_check_calls[-1][2], (
                "affection routine did not preserve its second-side turn at return",
                context, future, affection_check_calls
            )
            assert future[3:] == (
                0, 0, 0, 83, 0, 83, 0, 17, 0x11, 0, 0, 0
            ), (
                "state/counters changed between affection return and HandleFutureSight",
                context, future
            )
            assert len(future_sight_do_it_calls) == 2, (
                "HandleFutureSight did not execute .do_it exactly twice",
                context, future_sight_do_it_calls
            )
            assert {snap[2] for snap in future_sight_do_it_calls} == {0, 1}, (
                "HandleFutureSight did not visit one player and one enemy turn",
                context, future_sight_do_it_calls
            )
            for snap in future_sight_do_it_calls:
                assert snap[0:2] == (25, native), (
                    "Future Sight pass changed native identities", context, snap
                )
                assert snap[3:] == (
                    0, 0, 0, 83, 0, 83, 0, 17, 0x11, 0, 0, 0
                ), (
                    "Future Sight pass did not preserve zero counters/state",
                    context, snap
                )
            assert post_future_checkfaint_calls == [
                (
                    25, native, future_sight_do_it_calls[-1][2],
                    0, 0, 0, 83, 0, 83, 0x11, 0
                ),
            ], (
                "HandleFutureSight did not return from both zero-counter paths "
                "to the following CheckFaint with preserved state",
                context, post_future_checkfaint_calls
            )
            ...
            ) == bytes([0, 83]), ("enemy applydamage did not reduce player HP to 83", context)
            assert bytes(
            ...
            assert mem[addr("hBattleTurn")] == post_future_checkfaint_calls[0][2], context
            ...
            assert bytes(
                ...
                "enemy ResolveFaints did not write active player HP 83 back "
            ...
            assert (
                mem[addr("wBattleScriptBufferLoc")],
                mem[addr("wBattleScriptBufferLoc") + 1],
            ) == advance_script_pointer(read_script_snapshots[19], 19), (
                "enemy nineteenth script read did not advance the script pointer "
                "by exactly nineteen bytes",
                context,
                (
                    mem[addr("wBattleScriptBufferLoc")],
                    mem[addr("wBattleScriptBufferLoc") + 1],
            ...
            assert mem[addr("wMoveState")] == 0x11, (
                "ResetAbilityIgnorance did not clear both ability-ignorance bits",
                context, mem[addr("wMoveState")]
            )

            count += 1

        print(
            f"PASS: {count} native HandleFutureSight zero-counter -> CheckFaint cases; "
            "the real Future Sight routine visited both sides, both zero counters "
            "took the native RET Z path, and execution reached the following "
            "CheckFaint with native IDs, PP, Pressure, player/enemy party HP 83, "
            "damage 17, wMoveState $11, wBattleEnded=0, wEnemyFleeing=0, and "
            "weather 0 preserved"
        )
    finally:
