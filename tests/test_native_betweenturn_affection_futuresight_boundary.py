"""Native HandleAffectionSelfCure -> HandleFutureSight boundary regression.

Execute the validated player-first wild Tackle path through both real move
turns and into HandleBetweenTurnEffects through the second real
CheckFaint/ResolveFaints pass.

Then execute the real HandleAffectionSelfCure body. The deterministic affection
stub returns below AFFECTION_LEVEL_2 for both battlers, so each .do_it takes its
native early RET C without cure RNG or status mutation. Stop exactly at
HandleFutureSight entry before its body executes, while preserving native
identity, PP, Pressure, party HP, damage bookkeeping, wMoveState,
wBattleEnded, wEnemyFleeing, and weather state.
"""
import argparse
...
def main():
    ...
    def invoke(label, max_frames=128, stop_flag=None):
        ...
        # Controlled boundary sink used only when real HandleAffectionSelfCure
        # finishes both low-affection sides and reaches HandleFutureSight.
        ...
        if stop_flag is not None and stop_flag[0]:
            # The stop is at HandleFutureSight after HandleAffectionSelfCure
            # returns from both low-affection .do_it passes.
            assert mem[addr("hROMBank")] == symbols["HandleFutureSight"][0], label
        else:
    ...
    try:
        ...
        for native, species, form in cases:
            ...
            invoke(
                ...
                stop_flag=future_sight_stop_hit
            )

            assert future_sight_stop_hit[0], (
                "HandleAffectionSelfCure did not complete both low-affection "
                "passes and reach HandleFutureSight",
                context,
            )
            ...
            assert future[3:] == (
                ...
                "state changed between affection return and HandleFutureSight",
                context, future
            )
        ...
        print(
            f"PASS: {count} native HandleAffectionSelfCure -> HandleFutureSight cases; "
            "the real affection routine visited both sides, both low-affection "
            "checks returned before cure RNG, and execution reached HandleFutureSight "
            "with native IDs, PP, Pressure, player/enemy party HP 83, damage 17, "
            "wMoveState $11, wBattleEnded=0, wEnemyFleeing=0, and weather 0 preserved"
        )
    finally:
