"""Native enemy applydamage -> criticaltext boundary regression.
...
critical, damagestats, damagecalc, STAB, damagevariation, moveanim, and
failuretext commands. Then dispatch the enemy's real BattleCommand_applydamage
with validated varied damage 17 against the living 100-HP player target.

The presentation-only HP-bar/HUD boundaries remain stubbed exactly as on the
already-validated player applydamage path, while real damage survival checks,
TakeDamage, DealDamageToOpponent, and HP subtraction execute.

After applydamage returns, perform the fifteenth real enemy ReadMoveScriptByte
and stop after it returns criticaltext ($10), before enemy criticaltext
executes.
"""
import argparse
...
def main():
    ...
    try:
        ...
        for native, species, form in cases:
            ...
            assert read_script_calls == [True] * 34, (
                "expected the 19 validated player reads plus fifteen real enemy "
                "script-byte reads",
            ...
            assert len(read_script_calls) == 34, (
                "expected the validated 19 player reads plus fifteen enemy reads",
            ...
            assert mem[addr("wBattleEnded")] == 0x10, (
                "post-read boundary did not capture Tackle's fifteenth "
                "criticaltext command byte", context,
                mem[addr("wBattleEnded")]
            )
            assert bytes(
            ...
            assert (
                ...
                "enemy fifteenth script read did not advance the script pointer "
                "by exactly fifteen bytes",
                context,
                (
                    mem[addr("wBattleScriptBufferLoc")],
                    mem[addr("wBattleScriptBufferLoc") + 1],
        ...
        print(
            f"PASS: {count} native enemy applydamage -> criticaltext cases; "
            "real enemy Normal Tackle preserved native IDs, PP, Pressure and "
            "enemy-side state -> varied damage 17 -> real applydamage reduced "
            "player HP 100 -> 83 -> fifteenth real ReadMoveScriptByte -> stop "
            "before enemy criticaltext"
        )
    finally:
