"""Native enemy ResolveFaints -> DeferredSwitch boundary regression.

Execute the validated player-first wild Tackle path through the enemy's real
battle-command sequence, terminal $ff endmove helper chain, and PerformMove
cleanup tail. Then allow the enemy non-fainting ResolveFaints path to execute
for real.

ResolveFaints must write the active player HP 83 back to the player party,
preserve the enemy's already-written 83 HP, recognize that neither side has
fainted, and avoid faint animation, experience, and victory handling.

Stop exactly at the following enemy DeferredSwitch entry before its body
executes, with native identity, PP, Pressure, HP, and damage bookkeeping intact.
"""
import argparse
...
def main():
    ...
    def invoke(label, max_frames=128, stop_flag=None):
        ...
        # Controlled boundary sink used only by the enemy DeferredSwitch hook.
    ...
    def observe_deferred_switch(_):
        if resolve_active[0]:
            ...
            if enemy_resolve_stop_armed[0] and (mem[addr("hBattleTurn")] & 1):
                # Stop before the first DeferredSwitch instruction executes.
                # Redirect to a WRAM self-loop; invoke() recognizes the flag
                # and returns control to the harness without post-boundary work.
                enemy_deferred_stop_hit[0] = True
                regs.PC = 0xC110
    ...
    def setup_case(native, species, form):
        ...
        enemy_deferred_stop_hit[0] = False
        cleanup_active[0] = False
        resolve_active[0] = False
        usedmovetext_active[0] = False
    ...
    try:
        ...
        for native, species, form in cases:
            ...
            invoke("DoBattle", max_frames=256, stop_flag=enemy_deferred_stop_hit)

            assert enemy_deferred_stop_hit[0], (
                "enemy ResolveFaints did not reach DeferredSwitch boundary",
                context,
            )
        ...
        print(
            f"PASS: {count} native enemy ResolveFaints -> DeferredSwitch cases; "
            "real enemy Normal Tackle preserved native IDs, PP, Pressure, "
            "player HP 83 and damage 17 -> real enemy ResolveFaints wrote "
            "player party HP 83, preserved enemy party HP 83, took the "
            "non-fainting path, and stopped before DeferredSwitch body"
        )
    finally:
