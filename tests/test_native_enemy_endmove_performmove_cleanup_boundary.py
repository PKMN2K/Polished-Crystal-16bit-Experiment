"""Native enemy endmove -> PerformMove cleanup boundary regression.

Execute the validated player-first wild Tackle path through the enemy's real
checkobedience, usedmovetext, doturn, hastarget, checkhit, checkpriority,
critical, damagestats, damagecalc, STAB, damagevariation, moveanim,
failuretext, applydamage, criticaltext, neutral supereffectivetext,
non-fainting postfainteffects, and successful-hit posthiteffects commands.
Then allow the enemy's original terminal $ff endmove byte to run its real
DoTurn terminal helper chain: CheckEndMoveEffects -> CheckThroatSpray ->
CheckPowerHerb.

After CheckPowerHerb returns, stop exactly at PerformMove.end_protect before
the enemy PerformMove cleanup tail clears IN_ABILITY, ticks Disable/Encore,
clears Protect/Endure, refreshes the tilemap, or enters ResolveFaints.
"""
import argparse
...
def main():
    ...
    enemy_cleanup_stop_armed = [False]
    ...
    def setup_case(native, species, form):
        ...
        posthiteffects_active[0] = False
        enemy_posthiteffects_active[0] = False
        enemy_cleanup_stop_armed[0] = False
        cleanup_active[0] = False
        resolve_active[0] = False
        usedmovetext_active[0] = False
    ...
    try:
        ...
        for native, species, form in cases:
            ...
            assert bytes(
                ...
                "player party HP changed even though enemy PerformMove cleanup "
                "and ResolveFaints were intentionally not executed", context
            )
            assert bytes(
        ...
        print(
            f"PASS: {count} native enemy endmove -> PerformMove cleanup cases; "
            "real enemy Normal Tackle preserved native IDs, PP, Pressure, "
            "player HP 83 and damage 17 -> original $ff terminal endmove -> "
            "CheckEndMoveEffects -> CheckThroatSpray -> CheckPowerHerb -> "
            "stop at PerformMove.end_protect before enemy cleanup"
        )
    finally:
