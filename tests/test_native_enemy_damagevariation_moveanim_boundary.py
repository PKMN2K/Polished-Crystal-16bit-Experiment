"""Native enemy damagevariation -> moveanim boundary regression.
...
critical, damagestats, damagecalc, and STAB commands. Then dispatch the enemy's
real BattleCommand_damagevariation with the validated STAB damage 21.

BattleRandomRange(16) is deterministically forced to return 0, selecting the
85% floor and producing integer-truncated enemy damage 17 while preserving
native identities, enemy Tackle/PP state, and the player's Pressure state.

After damagevariation returns, perform the twelfth real enemy
ReadMoveScriptByte and stop after it returns moveanim ($6d), before enemy move
animation or HP application executes.
"""
import argparse
...
def main():
    ...
    def observe_damagevariation(_):
        ...
        if enemy:
            ...
            # Damagevariation has already been read and dispatched. Stop only
            # after the next read returns moveanim ($6d), before it dispatches.
            stop = [
                0xEA,
    ...
    try:
        ...
        for native, species, form in cases:
            ...
            assert read_script_calls == [True] * 31, (
                "expected the 19 validated player reads plus twelve real enemy "
                "script-byte reads",
            ...
            assert len(read_script_calls) == 31, (
                "expected the validated 19 player reads plus twelve enemy reads",
            ...
            assert mem[addr("wBattleEnded")] == 0x6D, (
                "post-read boundary did not capture Tackle's twelfth moveanim "
                "command byte", context,
                mem[addr("wBattleEnded")]
            )
            assert bytes(
            ...
            assert (
                ...
                "enemy twelfth script read did not advance the script pointer "
                "by exactly twelve bytes",
                context,
                (
                    mem[addr("wBattleScriptBufferLoc")],
                    mem[addr("wBattleScriptBufferLoc") + 1],
        ...
        print(
            f"PASS: {count} native enemy damagevariation -> moveanim cases; "
            "real enemy Normal Tackle preserved native IDs, PP, Pressure and "
            "enemy-side state -> STAB damage 21 -> deterministic 85% damage "
            "variation 17 -> twelfth real ReadMoveScriptByte -> stop before "
