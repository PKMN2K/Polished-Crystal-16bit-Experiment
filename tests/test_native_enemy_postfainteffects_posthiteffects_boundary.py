"""Native enemy postfainteffects -> posthiteffects boundary regression.
...
failuretext, applydamage, criticaltext, and neutral supereffectivetext
commands. Then dispatch the enemy's real BattleCommand_postfainteffects with
the player target alive at 83 HP and damage bookkeeping 17.

The real postfainteffects path must call HasOpponentFainted, recognize the
player target as alive, and return immediately without entering
RunFaintAbilities or any faint-side handling.

After postfainteffects returns, perform the eighteenth real enemy
ReadMoveScriptByte and stop after it returns posthiteffects ($13), before
enemy posthiteffects executes.
"""
import argparse
...
def main():
    ...
    def observe_postfainteffects(_):
        ...
        if enemy:
            ...
            # postfainteffects has already been read and dispatched. Stop only
            # after its real alive-target return and the following read yields
            # posthiteffects ($13), before posthiteffects dispatches.
            stop = [
                0xEA,
    ...
    try:
        ...
        for native, species, form in cases:
            ...
            assert read_script_calls == [True] * 37, (
                "expected the 19 validated player reads plus eighteen real enemy "
                "script-byte reads",
            ...
            assert len(read_script_calls) == 37, (
                "expected the validated 19 player reads plus eighteen enemy reads",
            ...
            assert mem[addr("wBattleEnded")] == 0x13, (
                "post-read boundary did not capture Tackle's eighteenth "
                "posthiteffects command byte", context,
                mem[addr("wBattleEnded")]
            )
            assert bytes(
            ...
            assert (
                ...
                "enemy eighteenth script read did not advance the script pointer "
                "by exactly eighteen bytes",
                context,
                (
                    mem[addr("wBattleScriptBufferLoc")],
                    mem[addr("wBattleScriptBufferLoc") + 1],
        ...
        print(
            f"PASS: {count} native enemy postfainteffects -> posthiteffects cases; "
            "real enemy Normal Tackle preserved native IDs, PP, Pressure, "
            "player HP 83 and damage 17 -> real postfainteffects checked the "
            "living target and returned without RunFaintAbilities -> eighteenth "
            "real ReadMoveScriptByte -> stop before enemy posthiteffects"
        )
    finally:
