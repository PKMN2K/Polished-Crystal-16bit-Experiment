"""Native enemy post-move return -> ProcessEnemyFleeing boundary regression.

Execute the validated player-first wild Tackle path through the enemy's real
battle-command sequence, PerformMove cleanup, non-fainting ResolveFaints,
no-op DeferredSwitch, and ResetAbilityIgnorance.

Then let BattleTurn.do_move execute its final wBattleEnded = 0 read, AND, and
RET for real. The caller's RET NZ must fall through, reaching
ProcessEnemyFleeing. Stop at that entry before its body executes, while proving
native identity, PP, Pressure, both party HP values at 83, damage bookkeeping
17, and wMoveState $11 remain intact.
"""
import argparse
...
def main():
    ...
    enemy_process_flee_stop_hit = [False]
    ...
    def invoke(label, max_frames=128, stop_flag=None):
        ...
        # Controlled boundary sink used only by the final enemy-turn
        # ProcessEnemyFleeing entry hook.
    ...
    def observe_process_enemy_fleeing(_):
        if (
            ...
            enemy_process_flee_stop_hit[0] = True
            regs.PC = 0xC110
            resolve_active[0] = False
    ...
    def setup_case(native, species, form):
        ...
        enemy_process_flee_stop_hit[0] = False
        cleanup_active[0] = False
        resolve_active[0] = False
        usedmovetext_active[0] = False
    ...
    try:
        ...
        for native, species, form in cases:
            ...
            invoke("DoBattle", max_frames=256, stop_flag=enemy_process_flee_stop_hit)

            assert enemy_process_flee_stop_hit[0], (
                "enemy post-move return did not reach ProcessEnemyFleeing boundary",
                context,
            )
            ...
            assert process_enemy_fleeing_calls == [
                ...
                "BattleTurn.do_move did not return cleanly through wBattleEnded=0 "
                "to the ProcessEnemyFleeing entry",
        ...
        print(
            f"PASS: {count} native enemy post-move -> ProcessEnemyFleeing cases; "
            "real enemy Normal Tackle preserved native IDs, PP, Pressure, "
            "player/enemy party HP 83 and damage 17; ResetAbilityIgnorance "
            "cleared wMoveState $55 to $11; final wBattleEnded=0 check returned "
            "cleanly to the ProcessEnemyFleeing entry"
        )
    finally:
