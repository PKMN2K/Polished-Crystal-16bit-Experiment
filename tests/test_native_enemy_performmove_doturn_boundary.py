"""Native enemy-side PerformMove -> DoTurn entry regression.

Execute the validated player-first wild Tackle path through the first real
PerformMove, non-fainting ResolveFaints, DeferredSwitch, and
ResetAbilityIgnorance. Seed the wild opponent's selected move as Tackle, flip
to hBattleTurn = 1, then let the opponent's PerformMove setup execute for real.
Verify native identities and enemy-perspective move state survive to DoTurn,
then stop before any opponent move-script command executes.
"""
import argparse
...
def main():
    ...
    do_turn_bank, do_turn_addr = symbols["DoTurn"]
    do_turn_original = data[offset("DoTurn"):offset("DoTurn") + 6]
    ...
    def observe_reset_ability_ignorance(_):
        if resolve_active[0]:
            ...
            for i, value in enumerate(stop):
                mem[do_turn_bank, do_turn_addr + i] = value
    ...
    def setup_case(native, species, form):
        ...
        for i, value in enumerate(do_turn_original):
            mem[do_turn_bank, do_turn_addr + i] = value
    ...
    try:
        ...
        for native, species, form in cases:
            ...
            assert mem[addr("wMoveState")] == 0x11, (
                "ResetAbilityIgnorance did not preserve non-ignorance bits",
        ...
        print(
            f"PASS: {count} native enemy PerformMove -> DoTurn cases; real "
            "player Tackle -> non-fainting ResolveFaints -> turn flip -> real "
            "enemy PerformMove with enemy Tackle -> DoTurn boundary stop "
            "before opponent move-script commands"
        )
    finally:
