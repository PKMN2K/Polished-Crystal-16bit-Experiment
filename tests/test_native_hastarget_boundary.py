"""Fourth real move-script command / hastarget native-identity regression.
...
UpdateMoveData, InitializeMove, Tackle's real checkobedience, usedmovetext and
doturn/PP-consumption commands, and then its real BattleCommand_hastarget path
against a living enemy. Terminate on the following script-byte read before
checkhit, accuracy/evasion logic, damage calculation, or HP application.

This proves both active native 16-bit identities survive target validation and
the opponent-ability lookup performed by a normal damaging move.
"""
import argparse
...
def main():
    ...
    def observe_read_script(_):
        ...
        # The fifth real script-byte read sees the test-only endturn byte
        # installed immediately after hastarget. End the outer battle loop
        # once DoTurn returns from that terminal boundary.
    ...
    def observe_target_ability(_):
        if hastarget_active[0]:
            ...
            target_ability_snapshots.append((
                read_native("wBattleMonNativeSpecies"),
                read_native("wEnemyMonNativeSpecies"),
                mem[addr("hBattleTurn")],
                mem[addr("wCurPlayerMove")],
    ...
    def setup_case(native, species, form):
        ...
        for calls in (
            ...
            checkhit_calls, damagecalc_calls, applydamage_calls,
    ...
    try:
        ...
        # doturn/BattleConsumePP and hastarget real. Tackle's NormalHit begins
        # with checkobedience, usedmovetext, doturn, hastarget, checkhit.
        # Replace only checkhit with endturn_command ($fe), so hastarget runs
        # normally against the living enemy and the fifth real script read
        # terminates before accuracy/evasion logic or damage.
        ...
        mem[normal_bank, normal_addr + 4] = 0xFE
        ...
        hook("BattleCommand_checkhit", lambda _: checkhit_calls.append(True))
        ...
        for native, species, form in cases:
            ...
            assert read_script_calls == [True, True, True, True, True], (
                "real move-script read count around first four commands", context,
                read_script_calls
            )
            ...
            assert not checkhit_calls, (
                "checkhit executed past terminal boundary", context
            )
        ...
        print(
            f"PASS: {count} native hastarget cases; real checkobedience -> "
            "usedmovetext -> doturn/BattleConsumePP -> hastarget living-target "
            "and ability checks -> fifth script-byte terminal boundary before "
            "checkhit, accuracy/evasion logic, damage, or HP change"
        )
    finally:
