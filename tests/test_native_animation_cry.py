"""Test battle animation native cry dispatch and WRAM restoration; audio playback is stubbed."""
import argparse
...
def main():
    ...
    calls = []
    waits = []
    anim_bank = syms["wPokeAnimStruct"][0]
    def capture(_):
        ...
        for turn in (0, 1):
            for native, species in cases:
                ...
                for name, value in (('wCurSpecies', 91), ('wCurForm', 0x23),
                                    ('wStereoPanningMask', 0), ('wCryTracks', 0xF0 if turn == 0 else 0x0F)):
                    mem[addr(name)] = value
                mem[addr('wCryPitch'):addr('wCryPitch') + 2] = [0x34, 0x12]
                mem[addr('wCryLength'):addr('wCryLength') + 2] = [0x78, 0x56]
                calls.clear()
                mem[0xFF70] = anim_bank
                mem[addr('wPokeAnimSpecies')] = 59
                mem[addr('wPokeAnimVariant')] = 1
                mem[addr('wPokeAnimSceneIndex')] = 0
                pointer = addr('PokeAnims.BattleSlow')
                mem[addr('wPokeAnimPointer'):addr('wPokeAnimPointer') + 2] = list(pointer.to_bytes(2, 'little'))
                run('TickPokeAnim')
                assert mem[addr('wPokeAnimSceneIndex')] == 1
                assert not waits
                mem[0xFF70] = 1
                ...
                assert mem[addr('wCryTracks')] == 0x0F
                count += 1
        # Non-battle slow/normal stereo scenes still use animation identity.
        for species, form in ((25, 1), (26, 2), (1, 0x21)):
            mem[0xFF70] = anim_bank
            mem[addr('wPokeAnimSpecies')] = species
            mem[addr('wPokeAnimVariant')] = form
            mem[addr('wPokeAnimSceneIndex')] = 0
            calls.clear()
            run('PokeAnim_StereoCry')
            assert mem[addr('wPokeAnimSceneIndex')] == 1
            assert not waits
            native_root = species + (256 if form & 0x20 else 0)
            p = offset('PokemonCryData') + (native_root - 1) * 5
            assert calls == [(data[p], int.from_bytes(data[p+1:p+3], 'little'), int.from_bytes(data[p+3:p+5], 'little'))]
            count += 1
        print(f'PASS: {count} native animation and legacy stereo-cry CPU cases')
    finally:
