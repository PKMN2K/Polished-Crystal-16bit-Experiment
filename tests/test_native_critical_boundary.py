"""Seventh real move-script command / critical native-identity regression.
...
doturn/PP-consumption, hastarget, checkhit and checkpriority commands, then
execute the real BattleCommand_critical eligibility/calculation path. Force
only affection below threshold and the final 0-23 critical roll to a
deterministic non-critical value, and terminate on the following script-byte
read before damagestats, damage calculation, or HP application.

This proves both active native 16-bit identities survive real critical-hit
eligibility, held-item and ability checks.
"""
import argparse
...
def main():
    ...
    def observe_read_script(_):
        ...
        # checkpriority boundary; and the seventh is the real critical
        # boundary. The eighth real script-byte read sees the test-only
        # endturn byte in place of damagestats and ends the outer battle loop.
    ...
    def setup_case(native, species, form):
        ...
        for calls in (
            ...
            user_valid_item_calls, damagestats_calls,
    ...
    try:
        ...
        # doturn/BattleConsumePP, hastarget, checkhit, checkpriority and
        # critical real. Replace only the following damagestats command with
        # endturn_command ($fe), so critical eligibility/calculation completes
        # and the eighth real script read terminates before damage-stat work.
        ...
        mem[normal_bank, normal_addr + 7] = 0xFE
        ...
        hook("BattleCommand_damagestats", lambda _: damagestats_calls.append(True))
        ...
        for native, species, form in cases:
            ...
            assert read_script_calls == [
                ...
                "real move-script read count around first seven commands", context,
                read_script_calls
            )
            ...
            assert not damagestats_calls, (
                "damagestats executed past terminal boundary", context
            )
        ...
        print(
            f"PASS: {count} native critical cases; real checkobedience -> "
            "usedmovetext -> doturn/BattleConsumePP -> hastarget -> checkhit -> "
            "checkpriority -> real critical reset/ability/item/affection/"
            "0-23 roll -> eighth script-byte terminal boundary before "
            "damagestats, damage calculation, or HP change"
        )
    finally:
