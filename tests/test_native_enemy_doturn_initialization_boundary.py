"""Native enemy-side DoTurn initialization regression.

Execute the validated player-first wild Tackle path through the first real
PerformMove and the enemy turn flip. Let the opponent's real PerformMove,
DoTurn, CheckTurn, UpdateMoveData, InitializeMove, and first
ReadMoveScriptByte execute with hBattleTurn = 1. Verify enemy Tackle and native
identities survive through script initialization, then stop immediately after
the first script byte returns and before any opponent battle-command body is
dispatched.
"""
import argparse
...
def main():
    ...
    def observe_read_script(_):
        ...
        if len(perform_move_calls) == 2 and (mem[addr("hBattleTurn")] & 1):
            enemy_first_read_snapshots.append((
                read_native("wBattleMonNativeSpecies"),
                read_native("wEnemyMonNativeSpecies"),
                mem[addr("hBattleTurn")],
                mem[addr("wCurPlayerMove")],
    ...
    def setup_case(native, species, form):
        ...
        for calls in (
            ...
            read_script_calls, read_script_snapshots, enemy_first_read_snapshots,
    ...
    try:
        ...
        for native, species, form in cases:
            ...
            assert read_script_calls == [True] * 20, (
                "expected the 19 validated player reads plus exactly one real "
            ...
            assert enemy_first_read_snapshots == [
                ...
                "enemy first ReadMoveScriptByte entry lost native identity, "
                "turn, move state, or script pointer",
                context, enemy_first_read_snapshots
            )
            ...
            assert (
                ...
                "enemy first script read did not advance the script pointer "
                "by exactly one byte",
                context,
                (
                    mem[addr("wBattleScriptBufferLoc")],
        ...
        print(
            f"PASS: {count} native enemy DoTurn initialization cases; real "
            "enemy PerformMove -> real DoTurn/CheckTurn/UpdateMoveData/"
            "InitializeMove -> first real ReadMoveScriptByte -> post-read "
            "boundary stop before opponent battle-command dispatch"
        )
    finally:
