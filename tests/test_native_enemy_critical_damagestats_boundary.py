"""Native enemy critical -> damagestats boundary regression.

Execute the validated player-first wild Tackle path through the enemy's real
checkobedience, usedmovetext, doturn, hastarget, checkhit, and checkpriority
commands. Then dispatch the enemy's real BattleCommand_critical through the
ordinary deterministic non-critical Tackle path with the player's validated
Pressure state intact. Verify native identities, enemy Tackle/PP state, ResetCrit,
Future Sight/item/ability checks, affection boundary, and the non-critical roll.
Finally execute the eighth real enemy ReadMoveScriptByte and stop after it
returns damagestats ($06), before enemy damage-stat calculation executes.
"""
import argparse
...
def main():
    ...
    def observe_critical(_):
        ...
        if enemy:
            ...
            # critical has already been read and dispatched. Stop after the
            # next real read returns Tackle's damagestats command ($06), so
            # the enemy critical body is real but damagestats is not dispatched.
            stop = [
                0xEA,
    ...
    try:
        ...
        for native, species, form in cases:
            ...
            assert read_script_calls == [True] * 27, (
                "expected the 19 validated player reads plus eight real enemy "
                "script-byte reads",
            ...
            assert mem[addr("wBattleEnded")] == 0x06, (
                "post-read boundary did not capture Tackle's eighth "
                "damagestats command byte", context,
                mem[addr("wBattleEnded")]
            )
            assert bytes(
            ...
            assert (
                ...
                "enemy eighth script read did not advance the script pointer "
                "by exactly eight bytes",
                context,
                (
                    mem[addr("wBattleScriptBufferLoc")],
                    mem[addr("wBattleScriptBufferLoc") + 1],
        ...
        print(
            f"PASS: {count} native enemy critical -> damagestats cases; "
            "real enemy deterministic non-critical Tackle critical command "
            "preserved native IDs, PP, Pressure and move state -> eighth real "
            "ReadMoveScriptByte -> stop before enemy damagestats logic"
        )
    finally:
