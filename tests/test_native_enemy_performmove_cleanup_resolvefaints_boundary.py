"""Native enemy PerformMove cleanup -> ResolveFaints boundary regression.

Execute the validated player-first wild Tackle path through the enemy's real
battle-command sequence and original terminal $ff endmove helper chain. Then
allow the enemy PerformMove cleanup tail to run for real.

Seed side-specific cleanup sentinels at the enemy CheckPowerHerb boundary.
The real cleanup must clear both IN_ABILITY bits, tick only the enemy user's
Disable/Encore counters, clear the player opponent's Protect/Endure bits, and
reach the real LoadTileMapToTempTileMap call site with native identity, PP,
Pressure, HP, and damage bookkeeping intact.

Stop exactly at the ResolveFaints entry before its body executes.
"""
import argparse
