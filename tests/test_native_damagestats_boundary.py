"""Eighth real move-script command / damagestats native-identity regression.
...
doturn/PP-consumption, hastarget, checkhit, checkpriority and critical
commands, then execute the real BattleCommand_damagestats physical-stat path.
Force only affection below threshold and the final 0-23 critical roll to a
deterministic non-critical value, and terminate on the following script-byte
read before damagecalc, later damage processing, or HP application.

This proves both active native 16-bit identities survive real Attack/Defense
selection, screen/modifier checks, level lookup, and damage-stat truncation.
"""
import argparse
...
def main():
    ...
    def observe_read_script(_):
        ...
        # and the eighth is the real damagestats boundary. The ninth real
        # script-byte read sees the test-only endturn byte in place of
        # damagecalc and ends the outer battle loop.
    ...
    try:
        ...
        # and damagestats real. Replace only the following damagecalc command
        # with endturn_command ($fe), so physical damage-stat setup completes
        # and the ninth real script read terminates before damage calculation.
        ...
        mem[normal_bank, normal_addr + 8] = 0xFE
        ...
        for native, species, form in cases:
            ...
            assert read_script_calls == [
                ...
                "real move-script read count around first eight commands", context,
                read_script_calls
            )
        ...
        print(
            f"PASS: {count} native damagestats cases; real checkobedience -> "
            "usedmovetext -> doturn/BattleConsumePP -> hastarget -> checkhit -> "
            "checkpriority -> critical -> physical damagestats Attack/Defense/"
            "level setup -> ninth script-byte terminal boundary before "
            "damagecalc, later damage processing, or HP change"
        )
    finally:
