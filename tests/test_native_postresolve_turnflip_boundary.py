"""Native post-ResolveFaints turn-flip regression.

Execute the validated first-turn wild Tackle path through real non-fainting
ResolveFaints, then let the real no-op DeferredSwitch and
ResetAbilityIgnorance tail complete. Verify only the two ability-ignorance bits
are cleared, BattleTurn flips to the opponent, and native identities survive.
Stop at the second PerformMove entry before the opponent move executes.
"""
import argparse
    ...
    try:
        ...
        for native, species, form in cases:
            ...
            assert second_perform_move_calls == [
                (25, native, 1, 0, 0, 0x11, 0),
            ], (
                "BattleTurn did not flip to the opponent after clearing only "
                "MOVESTATE_IGNOREABIL/MOVESTATE_OPP_IGNOREABIL",
