"""Move-order -> first PerformMove entry smoke regression.

Drive the validated wild BattleIntro -> DoBattle -> first BattleTurn path
through a deterministic Fight action and valid selected move. Keep
DetermineMoveOrder real with a deterministic player-first priority result, then
enter real PerformMove setup and stop at DoTurn before move effects or damage.

This proves the acting side's native 16-bit identity survives move ordering and
the first real move-execution setup boundary.
"""
import argparse
...
def main():
    ...
    def observe_do_turn(_):
        ...
        # End the battle loop after PerformMove returns from this terminal
        # boundary. No real move effect/damage code runs.
    ...
    try:
        ...
        # DoTurn is where actual move effects/damage begin. Stop there, after
        # the real initial PerformMove housekeeping has executed.
        install_stub("DoTurn", [0xC9], observe_do_turn)

        # When DoTurn returns, immediately return from PerformMove instead of
        # running post-move/faint resolution. wBattleEnded set by the callback
        # then makes BattleTurn exit after this first move setup.
        ...
        for native, species, form in cases:
            ...
            assert do_turn_calls == [True], (
                "DoTurn terminal boundary count", context, do_turn_calls
            )
        ...
        print(
            f"PASS: {count} native move-order/PerformMove-entry cases; "
            "real DetermineMoveOrder -> player-first PerformMove setup -> "
            "DoTurn terminal boundary with no move effects/damage"
        )
    finally:
