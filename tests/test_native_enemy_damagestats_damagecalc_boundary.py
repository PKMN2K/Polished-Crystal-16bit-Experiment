"""Native enemy damagestats -> damagecalc boundary regression.

Execute the validated player-first wild Tackle path through the enemy's real
checkobedience, usedmovetext, doturn, hastarget, checkhit, checkpriority, and
critical commands. Then dispatch the enemy's real BattleCommand_damagestats
through the physical Tackle Attack/Defense path with the player's validated
Pressure state intact. Verify native identities, enemy Tackle/PP state,
ResetDamage, opponent/user stat selection, Future Sight resolution, screen
handling, held-item/level party reads, and the resulting Attack/Defense/power/
level tuple. Finally execute the ninth real enemy ReadMoveScriptByte and stop
after it returns damagecalc ($50), before enemy damage calculation executes.
"""
import argparse
...
def main():
    ...
    def observe_damagestats(_):
        ...
        if enemy:
            ...
            # damagestats has already been read and dispatched. Stop after the
            # next read returns damagecalc ($50), before damagecalc dispatches.
            stop = [
                0xEA,
    ...
    try:
        ...
        for native, species, form in cases:
            ...
            assert len(read_script_calls) == 28, (
                "expected the validated 19 player reads plus nine enemy reads",
            ...
            assert mem[addr("wBattleEnded")] == 0x50, (
                "post-read boundary did not capture Tackle's ninth "
                "damagecalc command byte", context,
                mem[addr("wBattleEnded")]
            )
            assert bytes(
            ...
            assert (
                ...
                "enemy ninth script read did not advance the script pointer "
                "by exactly nine bytes",
                context,
                (
                    mem[addr("wBattleScriptBufferLoc")],
                    mem[addr("wBattleScriptBufferLoc") + 1],
        ...
        print(
            f"PASS: {count} native enemy damagestats -> damagecalc cases; "
            "real enemy physical Tackle damagestats preserved native IDs, PP, "
            "Pressure and enemy-side stat state -> ninth real "
            "ReadMoveScriptByte -> stop before enemy damagecalc logic"
        )
    finally:
