"""PerformMove -> DoTurn initialization boundary regression.

Drive the validated wild BattleIntro -> DoBattle -> first BattleTurn path
through deterministic move ordering and the first real PerformMove. Keep the
real DoTurn pre-move checks, UpdateMoveData, and InitializeMove path live, then
stop at the first move-script byte read before any Tackle effect command or
damage executes.

This proves both active native 16-bit identities survive into initialized move
execution, not just the PerformMove/DoTurn entry boundary.
"""
import argparse
...
def main():
    ...
    def observe_read_script(_):
        ...
        # The stub returns endturn_command ($fe), so no battle command is
        # dispatched. End the outer battle loop after DoTurn returns.
    ...
    try:
        ...
        # Keep DoTurn, CheckTurn, UpdateMoveData and InitializeMove real. Stop
        # only when DoTurn asks for its first move-script byte. Returning
        # endturn_command ($fe) prevents any effect command or damage routine
        # from running while still exercising initialization.
        install_stub(
            "ReadMoveScriptByte",
            [0x3E, 0xFE, 0xC9],
            observe_read_script,
        ...
        for native, species, form in cases:
            ...
            assert read_script_calls == [True], (
                "first move-script read boundary count", context,
                read_script_calls
            )
            assert read_script_snapshots[0][:5] == (25, native, 0, 33, 0), (
                "native/move state at first script read",
        ...
        print(
            f"PASS: {count} native DoTurn-initialization cases; "
            "real PerformMove -> DoTurn -> CheckTurn/UpdateMoveData/"
            "InitializeMove -> first script-byte boundary with no move "
            "effect command or damage"
        )
    finally:
