"""Native enemy STAB -> damagevariation boundary regression.
...
critical, damagestats, and damagecalc commands. Then dispatch the enemy's real
BattleCommand_stab with deterministic NORMAL/NORMAL attacker and target types,
neutral weather/items, the player's validated Pressure state, and enemy base
damage 14. Verify native identities, enemy Tackle/PP state, real type-matchup
handling, and 1.5x STAB damage 21. Finally execute the eleventh real enemy
ReadMoveScriptByte and stop after it returns damagevariation ($08), before
random damage variation executes.
"""
import argparse
...
def main():
    ...
    def observe_stab(_):
        ...
        if enemy:
            ...
            # STAB has already been read and dispatched. Stop after the next
            # read returns damagevariation ($08), before it dispatches.
            stop = [
                0xEA,
    ...
    try:
        ...
        for native, species, form in cases:
            ...
            # computes base damage 14, and the newly-real enemy STAB command
            # raises it to 21 before the controlled damagevariation boundary.
            assert bytes(
                mem[addr("wCurDamage"):addr("wCurDamage") + 2]
            ) == bytes([0, 21]), (
                "enemy STAB did not leave expected 21 damage", context,
            ...
            assert len(read_script_calls) == 30, (
                "expected the validated 19 player reads plus eleven enemy reads",
            ...
            assert mem[addr("wBattleEnded")] == 0x08, (
                "post-read boundary did not capture Tackle's eleventh "
                "damagevariation command byte", context,
                mem[addr("wBattleEnded")]
            )
            assert bytes(
            ...
            assert (
                ...
                "enemy eleventh script read did not advance the script pointer "
                "by exactly eleven bytes",
                context,
                (
                    mem[addr("wBattleScriptBufferLoc")],
                    mem[addr("wBattleScriptBufferLoc") + 1],
        ...
        print(
            f"PASS: {count} native enemy STAB -> damagevariation cases; "
            "real enemy Normal Tackle STAB preserved native IDs, PP, Pressure "
            "and enemy-side state -> base damage 14 to STAB damage 21 -> "
            "eleventh real ReadMoveScriptByte -> stop before damage variation"
        )
    finally:
