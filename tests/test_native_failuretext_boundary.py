"""Thirteenth real move-script command / failuretext native-identity regression.
...
damagestats, damagecalc, STAB, damagevariation and moveanim commands, then
execute real BattleCommand_failuretext.

Tackle is a successful hit in this fixture, so the real failuretext command
must take its no-failure early return with wAttackMissed == 0. The following
applydamage script byte is replaced with endturn_command, so no HP application
...
def main():
    ...
    def observe_read_script(_):
        ...
        # failuretext boundary. The fourteenth real script-byte read sees the
        # test-only endturn byte in place of applydamage and ends the outer
        # battle loop.
    ...
    try:
        ...
        # damagestats, damagecalc, STAB, damagevariation, moveanim and
        # failuretext real. Replace only the following applydamage command with
        # endturn_command ($fe), so the successful-hit failuretext early return
        # completes and the fourteenth real script read terminates before HP
        # application.
        ...
        mem[normal_bank, normal_addr + 13] = 0xFE
        ...
        for native, species, form in cases:
            ...
            assert read_script_calls == [
                ...
                "real move-script read count around first thirteen commands",
        ...
        print(
            f"PASS: {count} native failuretext cases; real checkobedience -> "
            "usedmovetext -> doturn/BattleConsumePP -> hastarget -> checkhit -> "
            "checkpriority -> critical -> damagestats -> damagecalc (14) -> "
            "STAB (21) -> damagevariation (17) -> moveanim -> real successful-"
            "hit failuretext early return -> fourteenth script-byte terminal "
            "boundary before applydamage or HP change"
        )
    finally:
