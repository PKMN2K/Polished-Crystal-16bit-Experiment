"""Native enemy moveanim -> failuretext boundary regression.
...
critical, damagestats, damagecalc, STAB, and damagevariation commands. Then
dispatch the enemy's real BattleCommand_moveanim with the validated varied
damage 17.

The presentation-only PlayFXAnimID boundary remains stubbed exactly as on the
already-validated player move-animation path, while the real moveanim control
flow (lowersub -> moveanimnosub -> raisesub) executes.

After moveanim returns, perform the thirteenth real enemy ReadMoveScriptByte
and stop after it returns failuretext ($0e), before enemy failuretext or HP
...
def main():
    ...
    def observe_moveanim(_):
        ...
        if enemy:
            ...
            # moveanim has already been read and dispatched. Stop only after
            # the following read returns failuretext ($0e), before it
            # dispatches, while leaving the real moveanim body untouched.
            stop = [
                0xEA,
    ...
    try:
        ...
        for native, species, form in cases:
            ...
            assert read_script_calls == [True] * 32, (
                "expected the 19 validated player reads plus thirteen real enemy "
                "script-byte reads",
            ...
            assert len(read_script_calls) == 32, (
                "expected the validated 19 player reads plus thirteen enemy reads",
            ...
            assert mem[addr("wBattleEnded")] == 0x0E, (
                "post-read boundary did not capture Tackle's thirteenth "
                "failuretext command byte", context,
                mem[addr("wBattleEnded")]
            )
            assert bytes(
            ...
            assert (
                ...
                "enemy thirteenth script read did not advance the script pointer "
                "by exactly thirteen bytes",
                context,
                (
                    mem[addr("wBattleScriptBufferLoc")],
                    mem[addr("wBattleScriptBufferLoc") + 1],
        ...
        print(
            f"PASS: {count} native enemy moveanim -> failuretext cases; "
            "real enemy Normal Tackle preserved native IDs, PP, Pressure and "
            "enemy-side state -> varied damage 17 -> real moveanim control "
            "flow -> thirteenth real ReadMoveScriptByte -> stop before enemy "
            "failuretext"
        )
    finally:
