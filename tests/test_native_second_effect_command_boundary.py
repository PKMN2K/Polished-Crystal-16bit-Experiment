"""Second real move-script command dispatch native-identity regression.
...
UpdateMoveData, InitializeMove, and Tackle's real checkobedience command. Let
the NormalHit script then dispatch its second real command,
BattleCommand_usedmovetext, including the real DisplayUsedMoveText state logic,
and terminate on the following script-byte read before doturn, targeting, hit
checks, damage calculation, or HP application can execute.

This proves both active native 16-bit identities survive a second actual battle
command and its normal state updates.
"""
import argparse
...
def main():
    ...
    def observe_read_script(_):
        ...
        # The third real script-byte read sees the test-only endturn byte
        # installed immediately after usedmovetext. End the outer battle loop
    ...
    def setup_case(native, species, form):
        ...
        for calls in (
            ...
            doturn_calls, hastarget_calls, damagecalc_calls, applydamage_calls,
    ...
    try:
        ...
        # ReadMoveScriptByte, checkobedience, usedmovetext, and
        # DisplayUsedMoveText state handling real. Tackle's NormalHit begins
        # with command bytes 2 (checkobedience), 3 (usedmovetext), 4 (doturn).
        # Replace only byte 2 with endturn_command ($fe), so the first two
        # commands execute normally and the third real script read terminates
        # before PP consumption, targeting, hit checks, or damage.
        ...
        mem[normal_bank, normal_addr + 2] = 0xFE
        ...
        hook("BattleCommand_doturn", lambda _: doturn_calls.append(True))
        ...
        for native, species, form in cases:
            ...
            assert read_script_calls == [True, True, True], (
                "real move-script read count around first two commands", context,
                read_script_calls
            )
            ...
            assert not doturn_calls, (
                "doturn executed past terminal boundary", context
            )
        ...
        print(
            f"PASS: {count} native second-command-dispatch cases; "
            "real checkobedience -> usedmovetext/DisplayUsedMoveText -> third "
            "script-byte terminal boundary before doturn, targeting, hit "
            "checks, damage, or HP change"
        )
    finally:
