"""Fifth real move-script command / checkhit native-identity regression.
...
doturn/PP-consumption and hastarget commands, then execute the real
BattleCommand_checkhit accuracy path. Neutralize only external shortcut inputs
so Tackle's 100%-accuracy calculation is deterministic, and terminate on the
following script-byte read before checkpriority, critical-hit handling, damage
calculation, or HP application.

This proves both active native 16-bit identities survive the real accuracy and
evasion resolution path.
"""
import argparse
...
def main():
    ...
    def observe_read_script(_):
        ...
        # The fifth read is the real checkhit command boundary; hastarget has
        # returned, so its instrumentation window can close. The sixth real
        # script-byte read sees the test-only endturn byte installed in place
        # of checkpriority and ends the outer battle loop.
    ...
    def observe_target_fainted(_):
        if hastarget_active[0]:
            ...
            target_fainted_snapshots.append((
                read_native("wBattleMonNativeSpecies"),
                read_native("wEnemyMonNativeSpecies"),
                mem[addr("hBattleTurn")],
                mem[addr("wEnemyMonHP")],
    ...
    def observe_user_ability(_):
        if checkhit_active[0]:
            ...
            accuracy_user_ability_snapshots.append((
                read_native("wBattleMonNativeSpecies"),
                read_native("wEnemyMonNativeSpecies"),
                mem[addr("hBattleTurn")],
                mem[addr("wCurPlayerMove")],
    ...
    def setup_case(native, species, form):
        ...
        for calls in (
            ...
            accuracy_random_calls, checkpriority_calls, critical_calls,
            damagecalc_calls, applydamage_calls,
    ...
    try:
        ...
        # doturn/BattleConsumePP, hastarget and checkhit real. Tackle's
        # NormalHit then reaches checkpriority. Replace only checkpriority with
        # endturn_command ($fe), so the real accuracy/evasion path completes
        # and the sixth real script read terminates before priority, critical or
        # damage processing.
        ...
        mem[normal_bank, normal_addr + 5] = 0xFE
        ...
        hook("BattleCommand_checkpriority", lambda _: checkpriority_calls.append(True))
        hook("BattleCommand_critical", lambda _: critical_calls.append(True))
        ...
        for native, species, form in cases:
            ...
            assert read_script_calls == [True, True, True, True, True, True], (
                "real move-script read count around first five commands", context,
                read_script_calls
            )
            ...
            assert not checkpriority_calls, (
                "checkpriority executed past terminal boundary", context
            )
            assert not critical_calls, (
                "critical-hit handling executed past terminal boundary", context
            )
        ...
        print(
            f"PASS: {count} native checkhit cases; real checkobedience -> "
            "usedmovetext -> doturn/BattleConsumePP -> hastarget -> real "
            "checkhit neutral-stage accuracy math/ability processing -> sixth "
            "script-byte terminal boundary before checkpriority, critical, "
            "damage, or HP change"
        )
    finally:
