"""Native enemy checkobedience -> usedmovetext boundary regression.

Execute the validated player-first wild Tackle path through the enemy's real
PerformMove, DoTurn initialization, and first ReadMoveScriptByte. Dispatch the
enemy's first real BattleCommand_checkobedience; because hBattleTurn = 1 it
must return immediately without player obedience handling. Then execute the
second real ReadMoveScriptByte, verify it returns usedmovetext ($03) with
native identities and enemy Tackle intact, and stop before the enemy
BattleCommand_usedmovetext body executes.
"""
import argparse
...
def main():
    ...
    def observe_checkobedience(_):
        ...
        if len(perform_move_calls) == 2 and (mem[addr("hBattleTurn")] & 1):
            ...
            # The first enemy command has already been read and dispatched.
            # Install the stop now so real checkobedience can return normally,
            # the loop performs the second real ReadMoveScriptByte, and the
            # returned usedmovetext byte ($03) is captured before dispatch.
            stop = [
                0xEA,
    ...
    try:
        ...
        for native, species, form in cases:
            ...
            assert read_script_calls == [True] * 21, (
                "expected the 19 validated player reads plus two real enemy "
                "script-byte reads",
            ...
            assert len(read_script_calls) == 21, (
                "expected the validated 19 player reads plus two enemy reads",
            ...
            assert mem[addr("wBattleEnded")] == 3, (
                "post-read boundary did not capture Tackle's second "
                "usedmovetext command byte", context,
                mem[addr("wBattleEnded")]
            )
            assert bytes(
            ...
            assert (
                ...
                "enemy second script read did not advance the script pointer "
        ...
        print(
            f"PASS: {count} native enemy checkobedience -> usedmovetext cases; "
            "real enemy first ReadMoveScriptByte -> real enemy checkobedience "
            "dispatch/return -> second real ReadMoveScriptByte -> post-read "
            "stop before enemy usedmovetext dispatch"
        )
    finally:
