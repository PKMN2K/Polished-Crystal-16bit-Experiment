"""Native endmove termination regression after the validated Tackle path.

Execute the validated first-turn wild Tackle path through real posthiteffects,
then leave the original endmove byte ($ff) untouched. Verify that DoMove takes
the normal terminal branch, reaches the three move-end helper boundaries, and
returns before unrelated PerformMove post-move/faint resolution.
"""
import argparse
...
def main():
    ...
    def observe_endmove_power_herb(_):
        ...
        # This is the harness-only outer-battle stop. It occurs only after the
        # original $ff endmove byte has selected the terminal helper chain.
    ...
    try:
        ...
        print(
            f"PASS: {count} native endmove cases; real "
            ...
            "real posthiteffects -> original $ff endmove -> "
            "move-end helper chain -> return before PerformMove tail"
        )
    finally:
