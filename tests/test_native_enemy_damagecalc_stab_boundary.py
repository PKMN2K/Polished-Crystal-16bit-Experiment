"""Native enemy damagecalc -> STAB boundary regression.
...
critical, and damagestats commands. Then dispatch the enemy's real
BattleCommand_damagecalc using the Attack/Defense/power/level tuple produced by
real enemy damagestats with the player's validated Pressure state intact.
Verify native identities, enemy Tackle/PP state and the neutral physical base
damage result. Finally execute the tenth real enemy ReadMoveScriptByte and stop
after it returns STAB ($07), before enemy STAB/type-matchup logic executes.
"""
import argparse
...
def main():
    ...
    def observe_damagecalc(_):
        ...
        if enemy:
            ...
            # damagecalc has already been read and dispatched. Stop after the
            # next read returns STAB ($07), before STAB dispatches.
            stop = [
                0xEA,
    ...
    try:
        ...
        for native, species, form in cases:
            ...
            assert read_script_calls == [True] * 28, (
                "expected the 19 validated player reads plus nine real enemy "
                "script-byte reads",
            ...
            # varied 17 damage. Enemy damagestats then resets that state and
            # the newly-real enemy damagecalc computes neutral Tackle base
            # damage 14 before the controlled STAB boundary.
            assert bytes(
                mem[addr("wCurDamage"):addr("wCurDamage") + 2]
            ) == bytes([0, 14]), (
                "enemy damagecalc did not leave expected base damage", context,
            ...
            assert len(read_script_calls) == 29, (
                "expected the validated 19 player reads plus ten enemy reads",
            ...
            assert mem[addr("wBattleEnded")] == 0x07, (
                "post-read boundary did not capture Tackle's tenth "
                "STAB command byte", context,
                mem[addr("wBattleEnded")]
            )
            assert bytes(
            ...
            assert (
                ...
                "enemy tenth script read did not advance the script pointer "
                "by exactly ten bytes",
                context,
                (
                    mem[addr("wBattleScriptBufferLoc")],
                    mem[addr("wBattleScriptBufferLoc") + 1],
        ...
        print(
            f"PASS: {count} native enemy damagecalc -> STAB cases; "
            "real enemy physical Tackle damagecalc preserved native IDs, PP, "
            "Pressure and enemy-side damage inputs -> neutral base damage 14 "
            "-> tenth real ReadMoveScriptByte -> stop before enemy STAB logic"
        )
    finally:
