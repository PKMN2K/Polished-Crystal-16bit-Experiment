"""Native enemy post-ProcessEnemyFleeing -> HandleBetweenTurnEffects boundary regression.

Execute the validated player-first wild Tackle path through both real move
turns, the enemy post-move return, and ProcessEnemyFleeing with
wEnemyFleeing = 0.

After ProcessEnemyFleeing takes its immediate RET Z, BattleTurn must execute
the following real ld a, [wBattleEnded], AND A, and RET NZ with
wBattleEnded = 0. RET NZ must not return. Stop exactly at the
HandleBetweenTurnEffects entry before its body executes, while proving native
identity, PP, Pressure, both party HP values at 83, damage bookkeeping 17,
wMoveState $11, and wEnemyFleeing = 0 remain intact.
"""
import argparse
...
def main():
    ...
    def invoke(label, max_frames=128, stop_flag=None):
        ...
        # Controlled boundary sink used only when the real enemy-turn path
        # reaches HandleBetweenTurnEffects after the post-flee wBattleEnded check.
    ...
    try:
        ...
        for native, species, form in cases:
            ...
            assert enemy_process_flee_return_stop_hit[0], (
                "BattleTurn did not reach HandleBetweenTurnEffects after the "
                "real zero wBattleEnded post-flee check",
                context,
            )
        ...
        print(
            f"PASS: {count} native enemy post-flee -> HandleBetweenTurnEffects cases; "
            "real enemy Normal Tackle preserved native IDs, PP, Pressure, "
            "player/enemy party HP 83 and damage 17; wMoveState remained $11; "
            "wEnemyFleeing=0 took the real immediate RET Z, then wBattleEnded=0 "
            "executed the real AND A / RET NZ fall-through and stopped exactly "
            "at HandleBetweenTurnEffects entry"
        )
    finally:
