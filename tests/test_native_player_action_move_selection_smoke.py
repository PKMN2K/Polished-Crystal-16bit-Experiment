"""Player action / move-selection boundary smoke regression.

Drive the validated wild BattleIntro -> DoBattle -> first BattleTurn path past
the first BattleMenu return with a deterministic Fight action. Keep
ParsePlayerAction real until MoveSelectionScreen is entered, then cancel at that
boundary and terminate on the second BattleMenu pass.

This proves both active 16-bit native identity shadows survive player action
dispatch and move-selection setup before move order or move execution begins.
"""
import argparse
...
def main():
    ...
    def observe_battle_menu(_):
        ...
        if len(battle_menu_calls) == 1:
            # Deterministic Fight action. BattleMenu returns carry clear via
            # its stub, so BattleTurn proceeds into real ParsePlayerAction.
            mem[addr("wBattlePlayerAction")] = 0
            mem[addr("wBattleEnded")] = 0
        else:
            # MoveSelectionScreen will return NZ (cancel), causing .loop1 to
            # revisit BattleMenu. End the smoke there before move ordering.
    ...
    def setup_case(native, species, form):
        ...
        for calls in (
            ...
            determine_order_calls, perform_move_calls,
    ...
    try:
        ...
        # First BattleMenu call selects Fight and returns carry clear. After a
        # deterministic move-selection cancel, the second call marks battle
        # ended so BattleTurn returns before DetermineMoveOrder.
        install_stub("BattleMenu", [0xA7, 0xC9], observe_battle_menu)

        # Enter MoveSelectionScreen through real ParsePlayerAction, snapshot
        # the setup state, then return NZ to represent a canceled selection.
        install_stub(
            "MoveSelectionScreen",
            [0x3E, 0x01, 0xB7, 0xC9],
            observe_move_selection,
        )
        ...
        hook("DetermineMoveOrder", lambda _: determine_order_calls.append(True))
        ...
        for native, species, form in cases:
            ...
            assert battle_menu_calls == [True, True], (
                "BattleMenu call count", context, battle_menu_calls
            )
            assert battle_menu_snapshots == [
                (25, native, 0, 1, 1, 1),
                (25, native, 0, 1, 1, 1),
            ], ("native/turn state across player menu loop", context, battle_menu_snapshots)
            ...
            assert not determine_order_calls, (
                "move order executed past boundary", context, determine_order_calls
            )
            assert not perform_move_calls, (
                "move execution reached past boundary", context, perform_move_calls
            )
        ...
        print(
            f"PASS: {count} native player-action/move-selection boundary cases; "
            "Fight -> real ParsePlayerAction -> MoveSelectionScreen cancel, "
            "with move order/execution not reached"
        )
    finally:
