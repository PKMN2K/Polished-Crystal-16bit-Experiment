"""Native enemy failuretext -> applydamage boundary regression.
...
critical, damagestats, damagecalc, STAB, damagevariation, and moveanim
commands. Then dispatch the enemy's real BattleCommand_failuretext with the
validated successful-hit state and varied damage 17.

The presentation-only PlayFXAnimID boundary remains stubbed exactly as on the
already-validated move-animation path. The successful failuretext command must
return without printing failure text or changing native/enemy-side state.

After failuretext returns, perform the fourteenth real enemy ReadMoveScriptByte
and stop after it returns applydamage ($0f), before enemy HP subtraction
executes.
"""
import argparse
...
def main():
    ...
    def observe_failuretext(_):
        ...
        if len(perform_move_calls) == 2 and (mem[addr("hBattleTurn")] & 1):
            ...
            # failuretext has already been read and dispatched. Stop only
            # after the following real read returns applydamage ($0f), before
            # applydamage dispatches or changes the player's HP.
            stop = [
                0xEA,
    ...
    try:
        ...
        for native, species, form in cases:
            ...
            assert read_script_calls == [True] * 33, (
                "expected the 19 validated player reads plus fourteen real enemy "
                "script-byte reads",
            ...
            assert len(read_script_calls) == 33, (
                "expected the validated 19 player reads plus fourteen enemy reads",
            ...
            assert mem[addr("wBattleEnded")] == 0x0F, (
                "post-read boundary did not capture Tackle's fourteenth "
                "applydamage command byte", context,
                mem[addr("wBattleEnded")]
            )
            assert bytes(
            ...
            assert (
                ...
                "enemy fourteenth script read did not advance the script pointer "
                "by exactly fourteen bytes",
                context,
                (
                    mem[addr("wBattleScriptBufferLoc")],
                    mem[addr("wBattleScriptBufferLoc") + 1],
        ...
        print(
            f"PASS: {count} native enemy failuretext -> applydamage cases; "
            "real enemy Normal Tackle preserved native IDs, PP, Pressure and "
            "enemy-side state -> varied damage 17 -> real moveanim and "
            "failuretext -> fourteenth real ReadMoveScriptByte -> stop before "
            "enemy applydamage"
        )
    finally:
