"""Native enemy usedmovetext -> doturn boundary regression.

Execute the validated player-first wild Tackle path through the enemy's real
checkobedience dispatch/return and second ReadMoveScriptByte. Dispatch the
enemy's real BattleCommand_usedmovetext and DisplayUsedMoveText, keeping only
the existing presentation-only ApplyTilemapInVBlank boundary stub. Verify the
enemy move-history, grammar, text-selection, native identities and Tackle state
remain correct, then execute the third real ReadMoveScriptByte and stop after
it returns doturn ($04), before the enemy BattleCommand_doturn body can consume
PP.
"""
import argparse
...
def main():
    ...
    def observe_usedmovetext(_):
        ...
        if len(perform_move_calls) == 2 and (mem[addr("hBattleTurn")] & 1):
            ...
            # The second enemy command has now been read and dispatched.
            # Install the post-read stop so real usedmovetext can complete,
            # return to DoMove, and the third real ReadMoveScriptByte can
            # return doturn ($04) without dispatching BattleCommand_doturn.
            stop = [
                0xEA,
    ...
    try:
        ...
        for native, species, form in cases:
            ...
            assert read_script_calls == [True] * 22, (
                "expected the 19 validated player reads plus three real enemy "
                "script-byte reads",
            ...
            assert len(read_script_calls) == 22, (
                "expected the validated 19 player reads plus three enemy reads",
            ...
            assert mem[addr("wBattleEnded")] == 4, (
                "post-read boundary did not capture Tackle's third "
                "doturn command byte", context,
                mem[addr("wBattleEnded")]
            )
            assert bytes(
            ...
            assert (
                ...
                "enemy third script read did not advance the script pointer "
                "by exactly three bytes",
                context,
                (
                    mem[addr("wBattleScriptBufferLoc")],
                    mem[addr("wBattleScriptBufferLoc") + 1],
        ...
        print(
            f"PASS: {count} native enemy usedmovetext -> doturn cases; real "
            "enemy usedmovetext/DisplayUsedMoveText state logic -> third real "
            "ReadMoveScriptByte -> post-read stop before enemy doturn/PP "
            "consumption"
        )
    finally:
