"""Native enemy checkhit -> checkpriority boundary regression.

Execute the validated player-first wild Tackle path through the enemy's real
checkobedience, usedmovetext, doturn, and hastarget commands. Then dispatch the
enemy's real BattleCommand_checkhit through the ordinary 100%-accuracy Tackle
path with neutral accuracy/evasion stages, deterministic randomness, and the
player's previously validated Pressure state intact. Verify native identities,
enemy Tackle state, ability lookups, accuracy math, and the hit result survive.
Finally execute the sixth real enemy ReadMoveScriptByte and stop after it
returns checkpriority ($0a), before enemy priority logic executes.
"""
import argparse
...
def main():
    ...
    def observe_checkhit(_):
        ...
        if enemy:
            # checkhit has already been read and dispatched. Install the stop
            # at the next read continuation so the real accuracy handler can
            # finish and the sixth enemy read can return checkpriority ($0a)
            # without dispatching BattleCommand_checkpriority.
            stop = [
                0xEA,
    ...
    try:
        ...
        for native, species, form in cases:
            ...
            assert len(read_script_calls) == 25, (
                "expected the validated 19 player reads plus six enemy reads",
            ...
            assert mem[addr("wBattleEnded")] == 0x0A, (
                "post-read boundary did not capture Tackle's sixth "
                "checkpriority command byte", context,
                mem[addr("wBattleEnded")]
            )
            assert bytes(
            ...
            assert (
                ...
                "enemy sixth script read did not advance the script pointer "
                "by exactly six bytes",
                context,
                (
                    mem[addr("wBattleScriptBufferLoc")],
                    mem[addr("wBattleScriptBufferLoc") + 1],
        ...
        print(
            f"PASS: {count} native enemy checkhit -> checkpriority cases; "
            "real enemy 100%-accuracy Tackle checkhit preserved native IDs, "
            "PP and Pressure and resolved a hit -> sixth real "
            "ReadMoveScriptByte -> stop before enemy checkpriority logic"
        )
    finally:
