"""Native enemy ProcessEnemyFleeing -> post-check boundary regression.

Execute the validated player-first wild Tackle path through both real move
turns and the enemy post-move return. Then execute ProcessEnemyFleeing for real
with wEnemyFleeing = 0.

The routine must load the zero flag, take its immediate RET Z, and return to
BattleTurn without entering EnemyCanFlee. Stop immediately after that return,
before BattleTurn reads wBattleEnded, while proving native identity, PP,
Pressure, both party HP values at 83, damage bookkeeping 17, and wMoveState
$11 remain intact.
"""
import argparse
...
def main():
    ...
    def invoke(label, max_frames=128, stop_flag=None):
        ...
        # Controlled boundary sink used only after the real enemy-turn
        # ProcessEnemyFleeing routine returns.
    ...
    def setup_case(native, species, form):
        ...
        for calls in (
            ...
            process_enemy_flee_return_calls, process_enemy_can_flee_calls,
    ...
    try:
        ...
        for native, species, form in cases:
            ...
            assert enemy_process_flee_return_stop_hit[0], (
                "real ProcessEnemyFleeing did not return to its post-call boundary",
                context,
            )
        ...
        print(
            f"PASS: {count} native enemy ProcessEnemyFleeing -> post-check cases; "
            "real enemy Normal Tackle preserved native IDs, PP, Pressure, "
            "player/enemy party HP 83 and damage 17; wMoveState remained $11; "
            "wEnemyFleeing=0 took the real immediate RET Z without EnemyCanFlee, "
            "then stopped before the following wBattleEnded read"
        )
    finally:
