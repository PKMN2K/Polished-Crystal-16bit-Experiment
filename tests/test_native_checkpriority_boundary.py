"""Sixth real move-script command / checkpriority native-identity regression.
...
doturn/PP-consumption, hastarget and checkhit commands, then execute the real
BattleCommand_checkpriority path for Tackle's normal priority. Verify the real
living-target, move-priority, Prankster and Soundproof checks, and terminate on
the following script-byte read before critical-hit handling, damage
calculation, or HP application.

This proves both active native 16-bit identities survive real move-priority and
priority-blocking checks.
"""
import argparse
...
def main():
    ...
    def observe_read_script(_):
        ...
        # returned, so its instrumentation window can close. The sixth read is
        # the real checkpriority boundary, so checkhit's window closes there.
        # The seventh real script-byte read sees the test-only endturn byte in
        # place of critical and ends the outer battle loop.
    ...
    def setup_case(native, species, form):
        ...
        for calls in (
            ...
            critical_calls, damagecalc_calls, applydamage_calls,
    ...
    try:
        ...
        # doturn/BattleConsumePP, hastarget, checkhit and checkpriority real.
        # Replace only the following critical command with endturn_command
        # ($fe), so the real normal-priority checks complete and the seventh
        # real script read terminates before critical or damage processing.
        ...
        mem[normal_bank, normal_addr + 6] = 0xFE
        ...
        for native, species, form in cases:
            ...
            assert read_script_calls == [
                ...
                "real move-script read count around first six commands", context,
                read_script_calls
            )
        ...
        print(
            f"PASS: {count} native checkpriority cases; real checkobedience -> "
            "usedmovetext -> doturn/BattleConsumePP -> hastarget -> checkhit -> "
            "real checkpriority living-target/GetMovePriority/Prankster/"
            "Soundproof checks -> seventh script-byte terminal boundary before "
            "critical, damage, or HP change"
        )
    finally:
