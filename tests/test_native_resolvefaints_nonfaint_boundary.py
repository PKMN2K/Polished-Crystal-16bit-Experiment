"""Native non-fainting ResolveFaints regression.

Execute the validated first-turn wild Tackle path through real PerformMove
cleanup, then let the non-fainting ResolveFaints path run for real. Verify both
party write-backs, the alive-opponent battle-over checks, and preserved native
identities. Stop at the first DeferredSwitch boundary after ResolveFaints
returns, before any opponent move can begin.
"""
import argparse
...
def main():
    ...
    def observe_resolve_return(_):
        if resolve_active[0]:
            native, species, form = current_case[0]
            resolve_return_calls.append((
                read_native("wBattleMonNativeSpecies"),
                read_native("wEnemyMonNativeSpecies"),
                mem[addr("hBattleTurn")],
                mem[addr("wBattleEnded")],
                mem[addr("wWhichMonFaintedFirst")],
                mem[addr("wPartyMon1HP")],
                mem[addr("wPartyMon1HP") + 1],
                mem[addr("wOTPartyMon1HP")],
                mem[addr("wOTPartyMon1HP") + 1],
            ...
            # Controlled stop before DeferredSwitch logic or the opponent move.
    ...
    def setup_case(native, species, form):
        ...
        for calls in (
            ...
            resolve_victory_music_calls, resolve_return_calls,
    ...
    try:
        ...
        install_stub("ApplyTilemapInVBlank", [0xC9])
        ...
        # ResolveFaints itself remains real. The first helper after PerformMove
        # returns is a controlled boundary so the enemy cannot take a move.
        install_stub("DeferredSwitch", [0xC9], observe_resolve_return)
        ...
        for native, species, form in cases:
            ...
            assert resolve_return_calls == [
                (25, native, 0, 0, 0, 0, 100, 0, 83, 34,
                 species, form, species, form),
            ], (
                "ResolveFaints did not return with correct write-back/native "
                "state before the opponent turn",
                context, resolve_return_calls
            )
            assert bytes(
            ...
            assert mem[addr("wPartyMon1PP")] == 34, (
                "player party PP write-back changed", context,
                mem[addr("wPartyMon1PP")]
            )
            assert perform_move_calls == [True], (
                "opponent PerformMove started after ResolveFaints boundary",
        ...
        print(
            f"PASS: {count} native non-fainting ResolveFaints cases; real "
            "Tackle through $ff endmove -> real PerformMove cleanup -> real "
            "ResolveFaints -> player/enemy party write-back -> three alive-"
            "enemy checks -> fit-player-party check -> return at DeferredSwitch "
            "boundary before opponent PerformMove"
        )
    finally:
