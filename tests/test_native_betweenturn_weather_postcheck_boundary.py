"""Native HandleWeather no-weather -> following CheckFaint boundary regression.

Execute the validated player-first wild Tackle path through both real move
turns, ProcessEnemyFleeing, the post-flee check, the first between-turn
CheckFaint/ResolveFaints pass, and then the real HandleWeather body.

With wBattleWeather = 0, HandleWeather must execute its native
ld a,[wBattleWeather], AND A, RET Z path without entering deeper weather
handling. Stop exactly at the following CheckFaint entry before that second
...
def main():
    ...
    enemy_process_flee_return_stop_hit = [False]
    ...
    def invoke(label, max_frames=128, stop_flag=None):
        ...
        # Controlled boundary sink used only when the real HandleWeather
        # no-weather return reaches the following CheckFaint entry.
        ...
        if stop_flag is not None and stop_flag[0]:
            # The stop is at CheckFaint, reached from HandleBetweenTurnEffects
            # after HandleWeather returns through its no-weather RET Z.
    ...
    def observe_between_turn_checkfaint(_):
        ...
        if weather_active[0]:
            ...
            enemy_process_flee_return_stop_hit[0] = True
            regs.PC = 0xC110
    ...
    def setup_case(native, species, form):
        ...
        enemy_process_flee_return_stop_hit[0] = False
        process_enemy_fleeing_active[0] = False
        between_turn_active[0] = False
        weather_active[0] = False
        cleanup_active[0] = False
    ...
    try:
        ...
        for native, species, form in cases:
            ...
            invoke(
                ...
                stop_flag=enemy_process_flee_return_stop_hit
            )

            assert enemy_process_flee_return_stop_hit[0], (
                "HandleWeather did not take its real no-weather return and "
                "reach the following CheckFaint boundary",
                context,
            )
            ...
            assert between_turn_resolve_faints_calls == [
                ...
                "initial between-turn CheckFaint did not execute real "
        ...
        print(
            f"PASS: {count} native HandleWeather no-weather -> CheckFaint cases; "
            "the real WEATHER_NONE path executed LD/AND/RET Z without deeper "
            "weather handling and reached the following CheckFaint with native "
            "IDs, PP, Pressure, player/enemy party HP 83, damage 17, wMoveState "
            "$11, wBattleEnded=0, and wEnemyFleeing=0 preserved"
        )
    finally:
