"""Native HandleBetweenTurnEffects initial CheckFaint -> HandleWeather boundary regression.

Execute the validated player-first wild Tackle path through both real move
turns, ProcessEnemyFleeing, the post-flee wBattleEnded check, and into the real
HandleBetweenTurnEffects body.

The first real CheckFaint must execute through ResolveFaints with both active
Pokémon still at 83 HP. Its carry result must be clear so the immediate RET C
is not taken. Stop exactly at HandleWeather entry before weather processing
begins, while preserving native identity, PP, Pressure, party HP, damage
bookkeeping, wMoveState, wBattleEnded, and wEnemyFleeing.
"""
import argparse
...
def main():
    ...
    def invoke(label, max_frames=128, stop_flag=None):
        ...
        # Controlled boundary sink used only when the real between-turn path
        # reaches HandleWeather after the initial CheckFaint returns carry clear.
        ...
        if stop_flag is not None and stop_flag[0]:
            # The stop is at HandleWeather, in the same bank as
            # HandleBetweenTurnEffects after the farcall has entered it.
            assert mem[addr("hROMBank")] == symbols["HandleWeather"][0], label
        else:
    ...
    def observe_between_turn_checkfaint(_):
        if between_turn_active[0]:
            between_turn_checkfaint_calls.append((
                read_native("wBattleMonNativeSpecies"),
                read_native("wEnemyMonNativeSpecies"),
                mem[addr("hBattleTurn")],
                mem[addr("wBattleEnded")],
    ...
    def setup_case(native, species, form):
        ...
        for calls in (
            ...
            handle_weather_calls, process_enemy_can_flee_calls,
    ...
    try:
        ...
        for native, species, form in cases:
            ...
            assert enemy_process_flee_return_stop_hit[0], (
                "HandleBetweenTurnEffects did not complete its initial "
                "CheckFaint and reach HandleWeather",
                context,
            )
            ...
            assert between_turn_checkfaint_calls == [
                (25, native, 1, 0, 0, 0, 83, 0, 83, 0x11),
            ], (
        ...
        print(
            f"PASS: {count} native HandleBetweenTurnEffects CheckFaint -> HandleWeather cases; "
            "the real initial CheckFaint/ResolveFaints path preserved native IDs, "
            "PP, Pressure, player/enemy party HP 83 and damage 17; carry stayed "
            "clear so RET C fell through, and execution stopped exactly at "
            "HandleWeather entry before weather processing"
        )
    finally:
