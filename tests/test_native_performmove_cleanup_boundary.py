"""Native PerformMove post-DoTurn cleanup regression.

Execute the validated first-turn wild Tackle path through the original endmove
byte, then let PerformMove continue through its real post-DoTurn cleanup.
Verify ability-state cleanup, the real TickDisableAndEncoreAfterMove update,
and target Protect/Endure reset, then stop exactly at ResolveFaints before
faint/experience/party write-back resolution.
"""
import argparse
...
def main():
    ...
    def observe_resolve_faints(_):
        ...
        # Harness-only outer-battle stop after all requested cleanup has run.
    ...
    try:
        ...
        # Stop at the first byte of ResolveFaints. Falling through to this
        # label proves all requested PerformMove cleanup has already run.
        install_stub("ResolveFaints", [0xC9], observe_resolve_faints)
        ...
        for native, species, form in cases:
            ...
            assert resolve_faints_calls == [
                ...
                "PerformMove cleanup did not reach ResolveFaints with preserved "
                "native identity, HP, damage, and cleanup state",
        ...
        print(
            f"PASS: {count} native PerformMove-cleanup cases; real "
            ...
            "supereffectivetext -> real non-fainting postfainteffects -> "
            "real posthiteffects -> original $ff endmove -> helper chain -> "
            "real PerformMove IN_ABILITY cleanup -> real disable/encore tick "
            "(5->4, 4->3) -> Protect/Endure reset -> ResolveFaints boundary"
        )
    finally:
