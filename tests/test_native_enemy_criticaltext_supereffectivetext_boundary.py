"""Native enemy criticaltext -> supereffectivetext boundary regression.
...
failuretext, and applydamage commands. Then dispatch the enemy's real
BattleCommand_criticaltext on the validated non-critical hit after player HP
has fallen from 100 to 83.

The real CheckCrit path must report non-critical, so criticaltext takes its
ordinary DelayFrames(20) path without printing critical-hit text or changing
native/enemy-side state.

After criticaltext returns, perform the sixteenth real enemy
ReadMoveScriptByte and stop after it returns supereffectivetext ($11), before
enemy supereffectivetext executes.
"""
import argparse
...
def main():
    ...
    def observe_criticaltext(_):
        ...
        if enemy:
            ...
            # criticaltext has already been read and dispatched. Stop only
            # after the next real script read returns supereffectivetext
            # ($11), before that command dispatches.
            stop = [
                0xEA,
                addr("wBattleEnded") & 0xff,
                addr("wBattleEnded") >> 8,
    ...
    try:
        ...
        for native, species, form in cases:
            ...
            assert read_script_calls == [True] * 35, (
                "expected the 19 validated player reads plus sixteen real enemy "
                "script-byte reads",
            ...
            assert len(read_script_calls) == 35, (
                "expected the validated 19 player reads plus sixteen enemy reads",
            ...
            assert mem[addr("wBattleEnded")] == 0x11, (
                "post-read boundary did not capture Tackle's sixteenth "
                "supereffectivetext command byte", context,
                mem[addr("wBattleEnded")]
            )
            assert bytes(
            ...
            assert (
                ...
                "enemy sixteenth script read did not advance the script pointer "
                "by exactly sixteen bytes",
                context,
                (
                    mem[addr("wBattleScriptBufferLoc")],
                    mem[addr("wBattleScriptBufferLoc") + 1],
        ...
        print(
            f"PASS: {count} native enemy criticaltext -> supereffectivetext cases; "
            "real enemy Normal Tackle preserved native IDs, PP, Pressure, "
            "player HP 83 and damage 17 -> real non-critical CheckCrit + "
            "DelayFrames(20) -> sixteenth real ReadMoveScriptByte -> stop "
            "before enemy supereffectivetext"
        )
    finally:
