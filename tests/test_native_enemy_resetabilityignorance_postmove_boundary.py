"""Native enemy ResetAbilityIgnorance -> post-move boundary regression.

Execute the validated player-first wild Tackle path through the enemy's real
battle-command sequence, PerformMove cleanup, non-fainting ResolveFaints, and
the real no-op DeferredSwitch with wDeferredSwitch = 0.

Then execute ResetAbilityIgnorance for real. It must clear only the two
ability-ignorance bits, changing the seeded wMoveState value from $55 to $11.
Stop exactly after ResetAbilityIgnorance returns, before BattleTurn.do_move
executes the following wBattleEnded read, while preserving native identity, PP,
Pressure, both party HP values at 83, and damage bookkeeping 17.
"""
import argparse
...
def main():
    ...
    #   ld a, [wBattleEnded]             ; +14  <-- controlled stop
    ...
    enemy_post_reset_stop_hit = [False]
    ...
    def invoke(label, max_frames=128, stop_flag=None):
        ...
        # Controlled boundary sink used only by the enemy
        # ResetAbilityIgnorance entry hook.
    ...
    def observe_reset_ability_ignorance(_):
        if resolve_active[0]:
            ...
            if enemy_resolve_stop_armed[0] and (mem[addr("hBattleTurn")] & 1):
                # Leave the enemy ResetAbilityIgnorance body entirely real.
                # The raw return-address hook below stops before the following
                # BattleTurn.do_move wBattleEnded read executes.
                return
    ...
    def observe_post_reset(_):
        if (
            ...
            enemy_post_reset_stop_hit[0] = True
            regs.PC = 0xC110
            resolve_active[0] = False
    ...
    def setup_case(native, species, form):
        ...
        enemy_post_reset_stop_hit[0] = False
        cleanup_active[0] = False
        resolve_active[0] = False
        usedmovetext_active[0] = False
        ...
        for calls in (
            ...
            post_reset_calls, second_perform_move_calls, enemy_move_read_calls,
    ...
    try:
        ...
        for native, species, form in cases:
            ...
            invoke("DoBattle", max_frames=256, stop_flag=enemy_post_reset_stop_hit)

            assert enemy_post_reset_stop_hit[0], (
                "enemy ResetAbilityIgnorance did not reach post-move boundary",
                context,
            )
            ...
            assert post_reset_calls == [
                ...
                "ability-ignorance bits before the post-move boundary",
        ...
        print(
            f"PASS: {count} native enemy ResetAbilityIgnorance -> post-move cases; "
            "real enemy Normal Tackle preserved native IDs, PP, Pressure, "
            "player/enemy party HP 83 and damage 17 -> real ResetAbilityIgnorance "
            "cleared wMoveState $55 to $11, then stopped before the following "
            "wBattleEnded read"
        )
    finally:
