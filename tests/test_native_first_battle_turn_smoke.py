"""Focused first-BattleTurn setup smoke regression for native identities.

Drive the real wild BattleIntro, DoBattle setup, and the real beginning of
BattleTurn through enemy-turn setup and the AI call boundaries until the player
BattleMenu entry. Deep AI decision implementations, unrelated item-effect
helpers, and actual user input are deterministic boundaries.

The regression proves both active 16-bit native identity shadows survive the
first real turn setup before any player action or move execution occurs.
"""
import argparse
...
def main():
    ...
    try:
        ...
        # BattleMenu is the player-input boundary. Carry set makes BattleTurn
        # return immediately after we snapshot both native identities.
        install_stub("BattleMenu", [0x37, 0xC9], observe_battle_menu)
        ...
        for native, species, form in cases:
            ...
            assert battle_menu_calls == [True], (
                "BattleMenu boundary count", context, battle_menu_calls
            )
            assert battle_menu_snapshots == [(25, native, 0, 1, 1, 1)], (
        ...
        print(
            f"PASS: {count} native first-BattleTurn setup smoke cases; "
            "enemy setup/AI boundaries -> player BattleMenu with both native shadows intact"
        )
    finally:
