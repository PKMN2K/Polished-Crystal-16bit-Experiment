"""Test native battle stereo-cry selection and parameters; audio playback is stubbed."""
import argparse
...
def main():
    ...
    def run(name, bc=0):
        ...
        regs.D, regs.E, regs.HL = 0x56, 0x78, 0xC222
        regs.SP, regs.PC = 0xC0FF, 0xC100
        pyboy.tick(4, False, False)
        assert (regs.PC, regs.SP) == (0xC104, 0xC0FF), (name, hex(regs.PC))
        assert (regs.B, regs.C, regs.D, regs.E, regs.HL) == (bc >> 8, bc & 255, 0x56, 0x78, 0xC222)
    ...
    try:
        ...
        for side, turn in ((0, 0), (0, 1), (1, 0), (1, 1)):
            for native, species in cases:
                for prefix, identity in (('wBattle', native if side == 0 else 143),
                                         ('wEnemy', native if side else 143)):
                ...
                calls.clear()
                run('PlayPlayerBattleStereoCry' if side == 0 else 'PlayEnemyBattleStereoCry')
                ...
                count += 1
        print(f'PASS: {count} native battle stereo-cry CPU cases')
    finally:
