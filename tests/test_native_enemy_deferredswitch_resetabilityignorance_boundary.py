"""Native enemy DeferredSwitch -> ResetAbilityIgnorance boundary regression.

Execute the validated player-first wild Tackle path through the enemy's real
battle-command sequence, PerformMove cleanup, and non-fainting ResolveFaints.
Then allow the enemy DeferredSwitch routine to execute for real with
wDeferredSwitch = 0.

DeferredSwitch must take its natural no-op return without entering
ForceDeferredSwitch. Stop exactly at the following ResetAbilityIgnorance entry
before its body executes, with native identity, PP, Pressure, both party HP
values at 83, damage bookkeeping 17, and the seeded move-state sentinel intact.
"""
import argparse
...
def main():
    ...
    def observe_reset_ability_ignorance(_):
        if resolve_active[0]:
            ...
            if enemy_resolve_stop_armed[0] and (mem[addr("hBattleTurn")] & 1):
                # Enemy DeferredSwitch has now returned naturally. Stop before
                # the first ResetAbilityIgnorance instruction executes.
                enemy_reset_stop_hit[0] = True
                regs.PC = 0xC110
    ...
    try:
        ...
        for native, species, form in cases:
            ...
            invoke("DoBattle", max_frames=256, stop_flag=enemy_reset_stop_hit)

            assert enemy_reset_stop_hit[0], (
                "enemy DeferredSwitch did not reach ResetAbilityIgnorance boundary",
                context,
            )
            ...
            assert mem[addr("wMoveState")] == 0x55, (
                "ResetAbilityIgnorance body executed before the boundary stop",
        ...
        print(
            f"PASS: {count} native enemy DeferredSwitch -> ResetAbilityIgnorance cases; "
            "real enemy Normal Tackle preserved native IDs, PP, Pressure, "
            "player/enemy party HP 83 and damage 17 -> real enemy DeferredSwitch "
            "took the wDeferredSwitch=0 no-op return, skipped ForceDeferredSwitch, "
            "and stopped before ResetAbilityIgnorance body"
        )
    finally:
