"""Native second between-turn CheckFaint -> HandleAffectionSelfCure boundary regression.

Execute the validated player-first wild Tackle path through both real move
turns, ProcessEnemyFleeing, the post-flee check, the initial between-turn
CheckFaint/ResolveFaints pass, and the real HandleWeather no-weather path.

Then execute the following CheckFaint through its real non-fainting
ResolveFaints path. Its carry result must remain clear, so RET C falls through
to HandleAffectionSelfCure. Stop exactly at HandleAffectionSelfCure entry before
its body executes, while preserving native identity, PP, Pressure, party HP,
damage bookkeeping, wMoveState, wBattleEnded, and wEnemyFleeing.
"""
import argparse
...
def main():
    ...
    between_turn_affection_stop_hit = [False]
    ...
    def invoke(label, max_frames=128, stop_flag=None):
        ...
        # Controlled boundary sink used only when the second real between-turn
        # CheckFaint returns carry clear and reaches HandleAffectionSelfCure.
        ...
        if stop_flag is not None and stop_flag[0]:
            # The stop is at HandleAffectionSelfCure, reached after the second
            # real CheckFaint/ResolveFaints pass returns carry clear.
            assert mem[addr("hROMBank")] == symbols["HandleAffectionSelfCure"][0], label
        else:
    ...
    def observe_handle_affection_self_cure(_):
        if between_turn_active[0] and post_weather_checkfaint_calls:
            ...
            between_turn_affection_stop_hit[0] = True
    ...
    def setup_case(native, species, form):
        ...
        between_turn_affection_stop_hit[0] = False
        process_enemy_fleeing_active[0] = False
        between_turn_active[0] = False
        weather_active[0] = False
        cleanup_active[0] = False
        ...
        for calls in (
            ...
            handle_affection_self_cure_calls, weather_deep_calls,
            process_enemy_can_flee_calls,
    ...
    try:
        ...
        for native, species, form in cases:
            ...
            invoke(
                ...
                stop_flag=between_turn_affection_stop_hit
            )

            assert between_turn_affection_stop_hit[0], (
                "second between-turn CheckFaint did not return carry clear and "
                "reach HandleAffectionSelfCure",
                context,
            )
        ...
        print(
            f"PASS: {count} native second CheckFaint -> HandleAffectionSelfCure cases; "
            "the real second CheckFaint/ResolveFaints path kept carry clear and "
            "fell through RET C to HandleAffectionSelfCure with native IDs, PP, "
            "Pressure, player/enemy party HP 83, damage 17, wMoveState $11, "
            "wBattleEnded=0, and wEnemyFleeing=0 preserved"
        )
    finally:
