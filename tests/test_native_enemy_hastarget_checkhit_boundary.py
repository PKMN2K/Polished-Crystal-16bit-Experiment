"""Native enemy hastarget -> checkhit boundary regression.

Execute the validated player-first wild Tackle path through the enemy's real
checkobedience, usedmovetext, and doturn commands. Then dispatch the enemy's
real BattleCommand_hastarget against a living player target with Pressure.
Verify the target-faint check and opponent-ability lookup use the enemy turn,
Pressure consumes exactly one additional enemy PP, and native identities stay
intact. Finally execute the fifth real enemy ReadMoveScriptByte and stop after
it returns checkhit ($09), before enemy accuracy/evasion logic executes.
"""
import argparse
...
def main():
    ...
    try:
        ...
        for native, species, form in cases:
            ...
            assert read_script_calls == [True] * 24, (
                "expected the 19 validated player reads plus five real enemy "
                "script-byte reads",
            ...
            assert len(read_script_calls) == 24, (
                "expected the validated 19 player reads plus five enemy reads",
            ...
            assert mem[addr("wBattleEnded")] == 0x09, (
                "post-read boundary did not capture Tackle's fifth "
                "checkhit command byte", context,
                mem[addr("wBattleEnded")]
            )
            assert bytes(
            ...
            assert (
                ...
                "enemy fifth script read did not advance the script pointer "
                "by exactly five bytes",
                context,
                (
                    mem[addr("wBattleScriptBufferLoc")],
                    mem[addr("wBattleScriptBufferLoc") + 1],
        ...
        print(
            f"PASS: {count} native enemy hastarget -> checkhit cases; real "
            "enemy hastarget verified living player target + Pressure lookup, "
            "Pressure decremented enemy active+OT PP 34->33 -> fifth real "
            "ReadMoveScriptByte -> stop before enemy checkhit accuracy logic"
        )
    finally:
