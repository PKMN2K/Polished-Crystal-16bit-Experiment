"""Native enemy doturn -> hastarget boundary regression.

Execute the validated player-first wild Tackle path through the enemy's real
checkobedience and usedmovetext commands, then dispatch the enemy's real
BattleCommand_doturn and BattleConsumePP. Verify Tackle consumes exactly one PP
from both enemy active and OT-party state without changing player PP or either
native identity. Then execute the fourth real enemy ReadMoveScriptByte and stop
after it returns hastarget ($3f), before the enemy target/Pressure command body
executes.
"""
import argparse
...
def main():
    ...
    def observe_doturn(_):
        ...
        if len(perform_move_calls) == 2 and (mem[addr("hBattleTurn")] & 1):
            ...
            # doturn has already been read and dispatched. Install the stop at
            # the next read continuation so real doturn/BattleConsumePP can
            # complete and the fourth real enemy read can return hastarget
            # ($3f) without dispatching BattleCommand_hastarget.
            stop = [
                0xEA,
    ...
    try:
        ...
        for native, species, form in cases:
            ...
            assert read_script_calls == [True] * 23, (
                "expected the 19 validated player reads plus four real enemy "
                "script-byte reads",
            ...
            assert consume_pp_calls == [True, True], (
                "player/enemy real BattleConsumePP call count",
            ...
            assert enemy_consume_pp_snapshots == [
                ...
                "enemy BattleConsumePP did not enter with isolated enemy PP",
            ...
            assert len(read_script_calls) == 23, (
                "expected the validated 19 player reads plus four enemy reads",
            ...
            assert mem[addr("wBattleEnded")] == 0x3F, (
                "post-read boundary did not capture Tackle's fourth "
                "hastarget command byte", context,
                mem[addr("wBattleEnded")]
            )
            assert bytes(
            ...
            assert mem[addr("wOTPartyMon1PP")] == 34, (
                "enemy OT-party PP was not decremented exactly once", context,
                mem[addr("wOTPartyMon1PP")]
            )
            assert mem[addr("wEnemyMonPP")] == 34, (
                "enemy active PP was not decremented exactly once", context,
                mem[addr("wEnemyMonPP")]
            )
            assert perform_move_calls == [True, True], (
            ...
            assert (
                ...
                "enemy fourth script read did not advance the script pointer "
        ...
        print(
            f"PASS: {count} native enemy doturn -> hastarget cases; real enemy "
            "doturn/BattleConsumePP decremented active+OT PP 35->34 while "
            "player PP stayed 34 -> fourth real ReadMoveScriptByte -> stop "
            "before enemy hastarget/Pressure handling"
        )
    finally:
