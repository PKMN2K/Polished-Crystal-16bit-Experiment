"""Native enemy posthiteffects -> endmove boundary regression.
...
failuretext, applydamage, criticaltext, neutral supereffectivetext, and
non-fainting postfainteffects commands. Then dispatch the enemy's real
BattleCommand_posthiteffects with the player target alive at 83 HP and damage
bookkeeping 17.

The command and its ordinary control flow remain real. As on the already
validated player-side posthiteffects regression, unrelated reactive
ability/item state is neutralized at the command-entry boundary so this
...
def main():
    ...
    try:
        ...
        print(
            f"PASS: {count} native enemy posthiteffects -> endmove cases; "
            "real enemy Normal Tackle preserved native IDs, PP, Pressure, "
            "player HP 83 and damage 17 -> real posthiteffects returned on the "
            "neutralized successful-hit path -> nineteenth real "
            "ReadMoveScriptByte -> stop before enemy endmove"
        )
    finally:
