"""Twelfth real move-script command / moveanim native-identity regression.
...
damagestats, damagecalc, STAB and damagevariation commands, then execute real
BattleCommand_moveanim control flow.

The visual PlayFXAnimID boundary is stubbed to an immediate return so this stays
a battle-state/native-identity regression rather than a full graphics/timing
test. The regression still verifies that the real moveanim command runs its
lowersub -> moveanimnosub -> raisesub chain and requests Tackle animation ID 33.

The following failuretext script byte is replaced with endturn_command, so
moveanim completes while failure text and HP application remain outside this
checkpoint.
"""
import argparse
...
def main():
    ...
    def observe_read_script(_):
        ...
        # the real moveanim boundary. The thirteenth real script-byte read
        # sees the test-only endturn byte in place of failuretext and ends the
        # outer battle loop.
    ...
    def setup_case(native, species, form):
        ...
        for calls in (
            ...
            failuretext_calls, applydamage_calls,
    ...
    try:
        ...
        # damagestats, damagecalc, STAB, damagevariation and moveanim real.
        # Replace only the following failuretext command with endturn_command
        # ($fe), so the move-animation control flow completes and the thirteenth
        # real script read terminates before failure-text handling.
        ...
        mem[normal_bank, normal_addr + 12] = 0xFE
        ...
        hook("BattleCommand_failuretext", lambda _: failuretext_calls.append(True))
        ...
        for native, species, form in cases:
            ...
            assert read_script_calls == [
                ...
                "real move-script read count around first twelve commands",
            ...
            assert not failuretext_calls, (
                "failuretext executed past terminal boundary", context
            )
        ...
        print(
            f"PASS: {count} native moveanim cases; real checkobedience -> "
            "usedmovetext -> doturn/BattleConsumePP -> hastarget -> checkhit -> "
            "checkpriority -> critical -> damagestats -> damagecalc (14) -> "
            "STAB (21) -> damagevariation (17) -> real moveanim control flow "
            "and Tackle animation request -> thirteenth script-byte terminal "
            "boundary before failuretext or HP change"
        )
    finally:
