"""Ninth real move-script command / damagecalc native-identity regression.
...
doturn/PP-consumption, hastarget, checkhit, checkpriority, critical and
damagestats commands, then execute the real BattleCommand_damagecalc path.

The synthetic fixture seeds nonzero Attack/Defense immediately before the real
damagestats command. The following STAB script byte is replaced with
endturn_command, so base damage calculation completes while STAB, damage
variation, animation and HP application remain outside this checkpoint.

This proves both active native 16-bit identities survive the real base damage
formula and that the validated damagestats outputs flow into damagecalc.
"""
import argparse
...
def main():
    ...
    def observe_read_script(_):
        ...
        # the eighth is the real damagestats boundary; and the ninth is the
        # real damagecalc boundary. The tenth real script-byte read sees the
        # test-only endturn byte in place of STAB and ends the outer battle loop.
    ...
    def setup_case(native, species, form):
        ...
        for calls in (
            ...
            damagecalc_result_snapshots, applydamage_calls,
    ...
    try:
        ...
        # damagestats and damagecalc real. Replace only the following STAB
        # command with endturn_command ($fe), so base damage calculation
        # completes and the tenth real script read terminates before STAB.
        ...
        mem[normal_bank, normal_addr + 9] = 0xFE
        ...
        for native, species, form in cases:
            ...
            assert read_script_calls == [
                ...
                "real move-script read count around first nine commands", context,
                read_script_calls
            )
            ...
            assert bytes(
                ...
                "damagecalc did not publish expected base damage", context,
        ...
        print(
            f"PASS: {count} native damagecalc cases; real checkobedience -> "
            "usedmovetext -> doturn/BattleConsumePP -> hastarget -> checkhit -> "
            "checkpriority -> critical -> damagestats -> real base damagecalc "
            "(14 damage) -> tenth script-byte terminal boundary before STAB, "
            "damage variation, or HP change"
        )
    finally:
