"""First real move-script command dispatch native-identity regression.
...
UpdateMoveData, and InitializeMove. Let Tackle's real NormalHit script read and
dispatch its first command, BattleCommand_checkobedience, then terminate on the
next real script-byte read before usedmovetext, damage calculation, or HP
application can execute.

This proves both active native 16-bit identities survive the first actual
battle-command dispatch, not just move initialization.
"""
import argparse
...
def main():
    ...
    def observe_read_script(_):
        ...
        # The second real script-byte read sees the test-only endturn byte
        # installed immediately after checkobedience. End the outer battle
        # loop once DoTurn returns from that terminal boundary.
        if len(read_script_calls) == 2:
    ...
    def setup_case(native, species, form):
        ...
        for calls in (
            ...
            usedmovetext_calls, damagecalc_calls, applydamage_calls,
    ...
    try:
        ...
        # ReadMoveScriptByte, and the first actual battle command real.
        # Tackle uses NormalHit: checkobedience is byte 0. Replace only byte 1
        # (usedmovetext) with endturn_command ($fe) in emulator memory. Thus
        # the first command dispatch executes normally and the next real
        # script read terminates before text, damage calculation, or HP writes.
        ...
        mem[normal_bank, normal_addr + 1] = 0xFE
        ...
        hook(
            ...
            lambda _: usedmovetext_calls.append(True),
        )
        ...
        for native, species, form in cases:
            ...
            assert read_script_calls == [True, True], (
                "real move-script read count around first command", context,
                read_script_calls
            )
            ...
            assert checkobedience_snapshots == [
                (
                    25, native, 0, 33,
                    read_script_snapshots[0][5] + 1 & 0xff,
                    read_script_snapshots[0][6]
                    + (1 if read_script_snapshots[0][5] == 0xff else 0),
                ),
            ], (
            ...
            assert read_script_snapshots[1][:5] == (25, native, 0, 33, 0), (
                "native/move state changed before post-command script read",
                context, read_script_snapshots
            )
            assert not usedmovetext_calls, (
                "usedmovetext executed past terminal boundary", context
            )
        ...
        print(
            f"PASS: {count} native first-command-dispatch cases; "
            "real PerformMove -> DoTurn -> InitializeMove -> real "
            "ReadMoveScriptByte -> BattleCommand_checkobedience -> second "
            "script-byte terminal boundary with no damage or HP change"
        )
    finally:
