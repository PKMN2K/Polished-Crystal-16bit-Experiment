"""Eleventh real move-script command / damagevariation native-identity regression.
...
damagestats, damagecalc and STAB commands, then execute real
BattleCommand_damagevariation.

The validated STAB fixture supplies 21 damage. BattleRandomRange(16) is
deterministically forced to return 0, so the real 85%-100% variation path must
choose 85% and publish integer-truncated damage 17.

The following moveanim script byte is replaced with endturn_command, so random
damage variation completes while move animation and HP application remain
...
def main():
    ...
    def observe_read_script(_):
        ...
        # eleventh is the real damagevariation boundary. The twelfth real
        # script-byte read sees the test-only endturn byte in place of moveanim
        # and ends the outer battle loop.
    ...
    def setup_case(native, species, form):
        ...
        for calls in (
            ...
            moveanim_calls, applydamage_calls,
    ...
    try:
        ...
        # damagestats, damagecalc, STAB and damagevariation real. Replace only
        # the following moveanim command with endturn_command ($fe), so real
        # 85%-100% damage variation completes and the twelfth real script read
        # terminates before move animation.
        ...
        mem[normal_bank, normal_addr + 11] = 0xFE
        ...
        hook("BattleCommand_moveanim", lambda _: moveanim_calls.append(True))
        ...
        for native, species, form in cases:
            ...
            assert read_script_calls == [
                ...
                "real move-script read count around first eleven commands",
            ...
            assert bytes(
                ...
                "damagevariation did not publish expected 17 damage", context,
            ...
            assert not moveanim_calls, (
                "move animation executed past terminal boundary", context
            )
        ...
        print(
            f"PASS: {count} native damagevariation cases; real "
            "checkobedience -> usedmovetext -> doturn/BattleConsumePP -> "
            "hastarget -> checkhit -> checkpriority -> critical -> "
            "damagestats -> damagecalc (14) -> STAB (21) -> real 85% "
            "damagevariation (17) -> twelfth script-byte terminal boundary "
            "before moveanim or HP change"
        )
    finally:
