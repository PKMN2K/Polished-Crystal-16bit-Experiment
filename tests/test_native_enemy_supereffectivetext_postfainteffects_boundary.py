"""Native enemy supereffectivetext -> postfainteffects boundary regression.
...
failuretext, applydamage, and criticaltext commands. Then dispatch the enemy's
real BattleCommand_supereffectivetext on the validated neutral Normal-vs-Normal
Tackle path with player HP 83 and damage bookkeeping 17.

The real neutral-effectiveness path must return immediately at EFFECTIVE,
without printing effectiveness text or entering Weakness Policy handling.

After supereffectivetext returns, perform the seventeenth real enemy
ReadMoveScriptByte and stop after it returns postfainteffects ($12), before
enemy postfainteffects executes.
"""
import argparse
...
def main():
    ...
    def observe_supereffectivetext(_):
        ...
        if enemy:
            ...
            # supereffectivetext has already been read and dispatched. Stop
            # only after its real neutral return and the following read yields
            # postfainteffects ($12), before postfainteffects dispatches.
            stop = [
                0xEA,
    ...
    try:
        ...
        for native, species, form in cases:
            ...
            assert read_script_calls == [True] * 36, (
                "expected the 19 validated player reads plus seventeen real enemy "
                "script-byte reads",
            ...
            assert len(read_script_calls) == 36, (
                "expected the validated 19 player reads plus seventeen enemy reads",
            ...
            assert mem[addr("wBattleEnded")] == 0x12, (
                "post-read boundary did not capture Tackle's seventeenth "
                "postfainteffects command byte", context,
                mem[addr("wBattleEnded")]
            )
            assert bytes(
            ...
            assert (
                ...
                "enemy seventeenth script read did not advance the script pointer "
                "by exactly seventeen bytes",
                context,
                (
                    mem[addr("wBattleScriptBufferLoc")],
                    mem[addr("wBattleScriptBufferLoc") + 1],
        ...
        print(
            f"PASS: {count} native enemy supereffectivetext -> postfainteffects cases; "
            "real enemy Normal Tackle preserved native IDs, PP, Pressure, "
            "player HP 83 and damage 17 -> real neutral supereffectivetext "
            "returned without text/item handling -> seventeenth real "
            "ReadMoveScriptByte -> stop before enemy postfainteffects"
        )
    finally:
